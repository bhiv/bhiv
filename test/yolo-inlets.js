...
describe('Yolo', function () {
  ...
  describe('Inlet', function () {
    ...
    it('should return 42', function (done) {
      ...
      A.setOlder(B);
      B.setOlder(C);
      A.on('test1', 'prepare', function (e, c) { c(null, [2]) });
      C.on('test1', 'prepare', function (e, c) { c(null, [4]) });
      B.on('test1', function (e, c) { c(null, e.concat(7)); });
    ...
    it('should return an error', function (done) {
      ...
      A.send(':test1', 'some', function (err) {
        if (err != 'this is an error') return done('Return an unexprected value');
        return done();
      });
    });
