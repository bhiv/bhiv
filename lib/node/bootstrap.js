...
global.toplevel = (function BhivBootstrap() {
  ...
  for (var i = 0; i < configChain.length; i++) {
    ...
    Bhiv.Util.merge(this.Syntaxes, config.Syntaxes);
  ...
  Bhiv.AST.Chain.prototype.load = function (name, prefix) {
    if (prefix == null) prefix = '';
    for (var i = 0; i < toplevel.paths.length; i++) {
      var root = toplevel.paths[i];
      var filepath = toplevel.Syntaxes[name] || name;
      var modulePath = [root, filepath].join('/');
      try {
        var module = require(modulePath).default;
      } catch (e) {
        // FIXME: if module has typo in a required location,
        //        it throw a module not found error too.
