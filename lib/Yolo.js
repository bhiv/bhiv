...
module.exports = new function Yolo() {
  ...
  this.Node.prototype.on = function (name, track, method) {
    if (arguments.length == 2) { method = track; track = 'execute' };
  ...
  this.Node.prototype._require = function (event, slice) {
    ...
    event.used.push(slice);
    return this.require.require(event.payload.fqn, event.payload.as, event.handler);
  };
  ...
  this.Node.prototype._create = function (event, slice) {
    ...
    this.create(fqn, fqn, function (err, node) {
  ...
  this.Node.prototype.create = function (fqn, as, callback) {
    ...
    return new Yolo.Event(this, fqn, payload).run(function (err, result) {
      if (err) return callback(err);
        ...
        return result.leaf.emit('-load', result, callback);
      } else {
