...
module.exports = new function Yolo() {
  ...
  this.Fqn = function Fqn(arg) {
    this.type = null;

    if (arguments.length == 0) {
      return this;
    } else if (arg instanceof Fqn) {
      return Fqn.clone(arg);
    } else if (typeof arg == 'string') {
      return Fqn.parse(arg);
    } else {
      throw new Error('Bad argument');
    }
  };

  this.Fqn.parse = (function (parser) {
    return function (string) {
      return Yolo.Fqn.clone(parser.parse(string));
    };
  })(require('./Yolo.Fqn.js'));

  this.Fqn.clone = function clone(model) {
    var replica = new Yolo.Fqn();
    replica.type = model.type;
    if (model.name != null)
      replica.name = model.name;
    if (model.args != null)
      replica.args = model.args.slice().map(clone);
    if (model.path != null)
      replica.path = model.path;
    if (model.next != null)
      replica.next = clone(model.next);
    return replica;
  };

  this.Fqn.prototype.map = function (iterator) {
    var list = [];
    for (var slice = this, i = 0; slice != null; slice = slice.next, i += 1)
      list.push(iterator(slice, i));
    return list;
  };

  this.Fqn.prototype.slice = function () {
    var root = new Yolo.Fqn();
    
  };

  this.Fqn.prototype.toString = function () {
    var next = this.next != null ? this.next.toString() : '';
    switch (this.type) {
    case 'root':
  ...
  this.Type = function (fqn) {
    if (!/^[A-Z]\w+(<[^>]+>)?(\.[A-Z]\w+(<[^>]+>)?)*$/.test(fqn))
      throw new Error('Bad Type name');
    this.fqn = fqn;
  };

  /** Event */
  this.Event = function Event(node, target, method, payload) {
    if (arguments.length != 4) throw new Error('All arguments are mandatory');
    if (!(node instanceof Yolo.Node)) throw new Error('Need a node to bring an event');
    if (!(typeof method == 'string')) throw new Error('Need a string method');

    this.node    = node;
    this.forward = typeof target == 'string' ? target.split('.').map(Event._typeChildName) : [];
    this.used    = [];
    this.method  = method;
  ...
  this.Event.prototype.next = function () {
    var event = this;
    if (this.forward.length > 0) {
      var forward = this.forward.shift();
      switch (forward.type) {
      case 'child':
        if (!this.did('child') && this.node.hasChild(forward)) {
          event.did('child', true);
          this.used.push(forward);
          return this.node.getChild(forward).give(this);
        } else if (this.node.parent == null && this.node.match(forward)) {
          return this.node.give(this);
        } else {
          this.forward.unshift(forward);
          if (!this.did('require') && this.node.require != null) {
            Array.prototype.unshift.apply(this.forward, this.used.splice(0, Infinity));
            var fqn = this.fqn();
            return this.node.create(fqn, fqn, function (err) {
              event.did('require', true);
              if (err) {
                event.error = err;
                return event.next();
              }
              event.did('child', false, '*');
              return event.next();
            });
          } else if (this.node.parent != null) {
            return this.node.parent.give(this);
          } else {
            if (this.error) this.node.emit('error', this.error);
            else debugger ;
          }
        }
        break ;
  ...
  this.Event.prototype.fqn = function () {
    var path = [];
    for (var i = 0; i < this.forward.length; i++) {
      var item = this.forward[i];
      if (item.type != 'child') break ;
      if (item.name != null) path.push(this.forward[i].name);
      else debugger;
    }
    return path.join('.');
  };
  ...
  this.Node.parsePattern = function (pattern) {
    var result = null;
    pattern.split('.').reverse().map(function (name) {
  ...
  this.Node.prototype.setRouter = function (router) {
    var originalGive = Yolo.Node.prototype.give;
    this.give = function (event) {
      return router.call(this, event, originalGive);
    };
  };

  this.Node.prototype.isRouter = function () {
    return this.give !== Yolo.Node.prototype.give;
  };
  ...
  this.Node.prototype._emit = function (event) {
    var flux = Yolo.Flux.create(event.handler, event.payload.payload);
    var stage = new Yolo.Node.Stage(event.payload.name, 'in', this, this, flux);
    return this.emit(event.payload.name, event.payload.payload, stage);
  };

  this.Node.prototype.emit = function (name, payload, stage) {
    if (!(stage instanceof Yolo.Node.Stage))
      return new Yolo.Event(this, null, 'emit', { name: name, payload: payload }).run(stage);
    var inlet = stage.layout.inlets[name];
    var track = '_' + stage.track
    if (inlet == null || !(inlet[track] instanceof Yolo.Node.Inlet))
      return stage.next(payload);
    var ns = Object.create(inlet[track], { node: { value: stage.node } });
    var flux = Yolo.Flux.create(stage.handler(), payload);
  ...
  this.Node.Stage.prototype.next = function (payload) {
    if (this.track == 'in') {
      if (this.layout.older instanceof Yolo.Node) {
        return this.then(this.layout.older).emit(this.name, payload, this);
      } else {
        return this.turn('out').emit(this.name, payload, this);
      }
    } else if (this.layout.newer instanceof Yolo.Node) {
      this.then(this.layout.newer).emit(this.name, payload, this);
    } else if (this.track == 'out') {
      return this.flux.emit('success', payload);
    } else {
      return this.flux.emit('fail', payload);
    }
  };
  ...
  this.Node.prototype.getChild = function (pattern) {
    if (typeof pattern == 'string') pattern = Yolo.Node.parsePattern(pattern);
    return this.fold(function (layout, node) {
      if (node != null) throw 'break';
      if (layout.children[pattern.name] == null) return null;
      if (pattern.next == null) return layout.children[pattern.name];
      return layout.children[pattern.name].getChild(pattern.next);
    }, null);
  };
  ...
  this.Node.prototype.create = function (fqn, as, callback) {
    ...
    var then = function (err, result) {
      if (err) return callback(err);
      else if (result.branch == null) return callback(null, result);
      else if (as != null) result.trunk.attach(result.branch);
    ...
    return new Yolo.Event(this, 'Root', 'require', payload).run(then);
  };

  this.Node.prototype._require = function (event) {
    if (this.require == null) return handler(this.cwd() + ': Unable to require');
  ...
  this.Node.prototype.send = function (path, inlet, payload, callback) {
    ...
    if (path.indexOf('.') === 0) path = this.cwd() + path;
    else if (path == '') path = null;
    return new Yolo.Event(this, path, 'emit', { name: inlet, payload: payload }).run(callback);
  };
  ...
  this.Node.prototype.match = function (pattern) {
    if (typeof pattern == 'string') pattern = Yolo.Node.parsePattern(pattern);
    var response = true;
    if (pattern.name != null && pattern.name != this.name) response = false;
    return response;
  };
  ...
  this.Node.prototype.give = function (event) {
    event.node = this;
    if (event.forward.length > 0) return event.next();
    var method = '_' + event.method;
  ...
  this.Loader.prototype.sanitize = function (item) {
    if (typeof item == 'string') item = { fqn: item };
    return item;
  };

  this.Loader.prototype.require = function (from, to, callback) {
    var chain = this.chain(from, to || from);
    return this.inflate(chain, this.node, null, null, callback);
  };

  this.Loader.prototype.chain = function (from, to) {
    var parts = new Yolo.Fqn(to);
    var procedure = parts.map(function (slice, i) {
      var fqn = parts.slice(0, i + 1).toString();
      return { name: slice.name, layouts: [{ fqn: fqn }] };
    });
    var layouts = from instanceof Array ? from.slice() : [from];
    layouts = layouts.map(this.sanitize);
    var last = procedure[procedure.length - 1];
    last.layouts = layouts;
    for (var i = 0; i < last.layouts.length; i++)
      if (last.layouts[i].fqn == to)
        return procedure;
    last.layouts.push(this.sanitize(to));
    return procedure;
  };

  this.Loader.prototype.inflate = function (chain, trunk, branch, leaf, callback) {
    if (leaf && leaf.isRouter()) {
      return callback(null, { trunk: trunk, branch: branch, leaf: leaf });
  ...
  this.Loader.prototype.createSegment = function (layouts, config, older, callback) {
    ...
    return this.instanciate(layout.fqn, function (err, newer) {
      if (err) return callback.call(this, err);
      if (newer != older) {
  ...
  this.Loader.prototype.generateData = function (layout) {
    var data = {};
    var paths = this.generateDataPaths(layout);
  ...
  this.Loader.prototype.generateDataPaths = function (layout) {
    var paths = [];
