...
module.exports = new function Yolo() {
  ...
  this.VM = new function () {
    ...
    this.AST = new function () {
      ...
      this._Waterfall.prototype.Map = function (source, glue) {
        return new AST._Waterfall(this, function () {
          var ast = { $: { type: 'map', source: source, glue: glue, block: this._ast } };
          return this._parent._push(ast);
        });
      };

      this._Waterfall.prototype.Fold = function (accu, source, glue) {
        return new AST._Waterfall(this, function () {
          var ast = { $: { type: 'fold', source: source, glue: glue, accu: accu, block: this._ast } };
          return this._parent._push(ast)._merger();
        });
      };

      this._Waterfall.prototype.Filter = function () {};

      this._Waterfall.prototype.Detect = function () {};
    ...
    this.Runtime.prototype.map = function (ast, data, callback) {
      ...
      if (source == null || source == '.') source = data;
      else if (typeof source == 'string') source = Yolo.Util.getIn(data, source);
      else if (source instanceof Array) /* source is source */;
      else callback(new Error('Bad source'));
      var glue = ast.glue || '$:value';
      return Yolo.Async.map(source, function (value, callback, index) {
        var flow = { flow: data, key: index, value: value };
        return runtime.run(glue, flow, function (err, subflow) {
          if (err) return callback(err);
        ...
        if (err) return callback(err);
        var source = ast.source;
        if (source == null || source == '.') return callback(null, result);
        if (ast.source instanceof Array) return callback(null, result);
        Yolo.Util.setIn(data, source, result);
        return callback(null, data);
      });
    };
