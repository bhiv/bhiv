...
module.exports = new function Yolo() {
  ...
  this.Async = new function () {
    ...
    this.fold = function fold(source, accumulator, iterator, callback) {
      if (actions.length < 1) return callback(null, accumulator);
  ...
  this.Node.prototype.on = function (name, track, method) {
    ...
    if (typeof method == 'function') method = new Yolo.Node.Method(method);
  ...
  this.Node.prototype.attach = function (child, name) {
    ...
    if (older != null) {
      ...
      else throw new Error('Child with the same name already attached');
    }
    child.name = name;
    child.parent = this;
  ...
  this.Node.prototype._field = function (event, slice) {
    if (this.model.fields == null) {
      event.node.send('Log:error', event.node.cwd() + ': has no fields');
      return event.forward.splice(0, Infinity);
    } else if (this.model.fields[slice.name] == null) {
      event.node.send('Log:error', event.node.cwd() + ': has no field "' + slice.name + '"');
      return event.forward.splice(0, Infinity);
    } else if (this.model.fields[slice.name].node == null) {
      event.node.send('Log:error', event.node.cwd() + ': field "' + slice.name + '" not initialized');
      return event.forward.splice(0, Infinity);
    } else {
  ...
  this.Node.prototype.resolve.dollar = function (action, data, callback) {
    if (action._ != null) {
      return this.resolve.underscore.call(this, action, data, callback);
      ...
      return this.resolve.map(action, data, callback);
    }
  };

  this.Node.prototype.resolve.underscore = function (action, data, callback) {
    var node = this;
    return this.resolve(action._, data, function (err, data) {
      if (err) return callback(err);
      if ('$' in action) return node.resolve.dollar.call(node, { $: action.$ }, data, callback);
      else return callback(null, { _: data });
    });
  };
