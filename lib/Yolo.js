...
module.exports = new function Yolo() {
  ...
  this.Async = new function () {
    ...
    this.fold = function fold(source, accumulator, iterator, callback, index) {
      if (index == null || index < 0) index = 0;
      if (index >= source.length) return callback(null, accumulator);
      return iterator(source[index], accumulator, function (err, accumulator) {
        if (err) return callback(err, accumulator);
        return fold(source, accumulator, iterator, callback, index + 1);
      }, index);
    };
  ...
  this.VM = new function () {
    ...
    this.AST = new function () {
      ...
      this._Waterfall.prototype._push = function (ast) {
        this._current = ast;
        if (this._ast === NULL)
          this._ast = ast;
        else if (typeof this._ast === 'object' && this._ast.$ instanceof Array)
          this._ast.$.push(ast);
        else
          this._ast = { $: [this._ast, ast] };
        return this;
      };

      this._Waterfall.prototype._merger = function () {
        var self = Object.create(this);
        var merger = new AST._Merger(this);
        for (var method in merger) self[method] = merger[method];
        return self;
      };

      this._Waterfall.prototype.relocate = function (location) {};

      this._Waterfall.prototype.as = function (glue) {
        if (typeof glue != 'object' || glue == null) glue = { _: glue };
        return this._push(glue)._merger();
      };

      this._Waterfall.prototype.put = function (glue) {
        return this._push({ __: glue })._merger();
      };

      this._Waterfall.prototype.then = function (identifier, glue) {
        assert.isIdentifier(identifier);
        var ast = { $: identifier };
        if (arguments.length > 1) ast._ = glue;
        return this._push(ast)._merger();
      };

      this._Waterfall.prototype.apply = function (path, identifier) {
        assert.isPath(path);
        assert.isIdentifier(identifier);
        var ast = { $: identifier, _: '$:' + path };
        return this._push(ast)._merger().merge(path);
      };

      this._Waterfall.prototype.defer = function (identifier, glue) {
        assert.isIdentifier(identifier);
        var block = { $: identifier };
        if (arguments.length > 1) block._ = glue;
        var ast = { $: { type: 'sideway', block: block } };
        return this._push(ast);
      };

      /** Control Flow */

      this._Waterfall.prototype.Race = function (holder) {
        return new AST._Race(this, function () {
          return this._parent._push(this._ast)._merger();
        }, holder);
      };

      this._Waterfall.prototype.Until = function (test, delay, glue) {
        if (arguments.length < 2) delay = 0;
        else delay |= 0;
        var hasGlue = arguments.length > 2;
        return new AST._Waterfall(this, function () {
          var ast = { $: { type: 'until', test: test, delay: delay, block: this._ast } };
          if (hasGlue) ast.$.glue = glue;
          return this._parent._push(ast);
        });
      };

      this._Waterfall.prototype.Sideway = function () {
        return new AST._Waterfall(this, function () {
          var ast = { $: { type: 'sideway', block: this._ast } };
          return this._parent._push(ast);
        });
      };
      ...
      this._Waterfall.prototype.Map = function (source) {
        assert.isCollection(source);
        return new AST._Waterfall(this, function () {
          var ast = { $: { type: 'map', source: source, block: this._ast } };
          return this._parent._push(ast)._merger();
        });
      };

      this._Waterfall.prototype.Reduce = function (source) {
        assert.isCollection(source);
        return new AST._Waterfall(this, function () {
          var ast = { $: { type: 'reduce', source: source, block: this._ast } };
          return this._parent._push(ast)._merger();
        });
      };

      this._Waterfall.prototype.Filter = function (source) {
        assert.isCollection(source);
        return new AST._Waterfall(this, function () {
          var ast = { $: { type: 'filter', source: source, block: this._ast } };
          return this._parent._push(ast)._merger();
        });
      };

      this._Waterfall.prototype.Detect = function (source) {
        return new AST._Waterfall(this, function () {
          var ast = { $: { type: 'detect', source: source, block: this._ast } };
          return this._parent._push(ast)._merger();
        });
      };

      this._Waterfall.prototype.Fold = function (accu, source) {
        assert.isCollection(source);
        return new AST._Waterfall(this, function () {
          var ast = { $: { type: 'fold', source: source, accu: accu, block: this._ast } };
          return this._parent._push(ast)._merger();
        });
      };

      /** Flow routing */
      this._Waterfall.prototype.Match = function (view) {
        var hasView = arguments.length > 0;
        return new AST._Match(view, this, function () {
          var ast = { $: { type: 'match' } };
          if (hasView) ast.$.match = this._view;
          ast.$.cases = this._cases;
          ast.$.otherwise = this._otherwise;
          return this._parent._push(ast)._merger();
        });
      };

      this._Waterfall.prototype.Unless = function (test) {
        return new AST._Waterfall(this, function () {
          var ast = { $: { type: 'unless', test: test, block: this._ast } };
          return this._parent._push(ast);
        });
      };

      this._Waterfall.prototype.memoize = function (duration, key) {
        return new AST._Waterfall(this, function () {
          var ast = { $: { type: 'memoize', expire: duration, key: key, then: this._ast } };
          var context = this._parent._push(ast);
          return context.end.apply(context, arguments);
        });
      };

      this._Waterfall.prototype.shunt = function (test, then) {
        return new AST._Waterfall(this, function () {
          var ast = { $: { type: 'shunt', test: test, then: then, block: this._ast } };
          var context = this._parent._push(ast);
          return context.end.apply(context, arguments);
        });
      };

      this._Waterfall.prototype.trap = function (test, then) {
        return new AST._Waterfall(this, function () {
          var ast = { $: { type: 'trap', test: test, then: then, block: this._ast } };
          var context = this._parent._push(ast);
          return context.end.apply(context, arguments);
        });
      };

      this._Waterfall.prototype.assert = function (test, then) {
        return new AST._Waterfall(this, function () {
          var ast = { $: { type: 'assert', test: test, then: ast, block: this._ast } };
          var context = this._parent._push(ast);
          return context.end.apply(context, arguments);
        });
      };

      this._Waterfall.prototype.failWith = function (message) {
        var ast = { $: { type: 'failWith', error: message } };
        return this._push(ast);
      };

      this._Waterfall.prototype.Block = function (name) {
        return new AST._Waterfall(this, function () {
          var ast = { $: { type: 'block', name: name, block: this._ast } };
          return this._parent._push(ast);
        });
      };

      this._Waterfall.prototype.Then = function (fqn, glue) {
        assert.isFqn(fqn);
        var ast = { $: fqn };
        if (arguments.length > 1) ast._ = glue;
        return new AST._MetaCall(this, function () {
          for (var key in this._changes) break ;
          if (key) ast.$changes = this._changes;
          return this._parent._push(ast)._merger();
        });
      };

      /* Merger */
      this._Merger = function (parent) {
        this._unwrapped = parent;
      };

      this._Merger.prototype.wrap = function (glue) {
        this._current.$$ = { type: 'merge', mode: 'wrap', glue: glue };
        return this._unwrapped;
      };

      this._Merger.prototype.merge = function (path) {
        if (arguments.length == 0) {
          this._current.$$ = { type: 'merge', mode: 'root' };
        } else {
          this._current.$$ = { type: 'merge', mode: 'into', to: path };
        }
        return this._unwrapped;
      };

      this._Merger.prototype.replace = function (path) {
        this._current.$$ = { type: 'merge', mode: 'set', to: path };
        return this._unwrapped;
      };

      /* Race */
      this._Race = function (parent, end, holder) {
        this._ast      = holder || {};
        this._parent   = parent;
        this._branches = [];
        this.end       = end;
      ...
      this._Race.prototype._isConflict = function (path) {
        var pattern = path + '.';
      ...
      this._Race.prototype.At = function (path, glue) {
        if (this._isConflict(path))
          throw new Error('Race: ' + path + ' already locked by an other branch');
        if (!(arguments.length > 1)) glue = '$:' + path;
        var waterfall = new AST._Waterfall(this, function () {
          var context = this._close();
          return context.end.apply(context, arguments);
        });
        waterfall._close = function () {
          Yolo.Util.setIn(this._parent._ast, path, this._ast);
          return this._parent;
        };
        waterfall.At = function () {
          var self = this._close();
      ...
      /* Match */
      this._Match = function (view, parent, end) {
        this._view      = view;
        this._cases     = [];
      ...
      this._Match.prototype._bind = function (waterfall) {
        var proto = AST._Match.prototype;
      ...
      this._Match.prototype.When = function (test) {
        ...
        var waterfall = new AST._Waterfall(this, function () {
          this._parent._cases.push({ type: type, test: test, block: this._ast });
          return this._parent;
        });
        this._bind(waterfall);
        return waterfall;
      };

      this._Match.prototype.WhenEquiv = function (value) {
        var waterfall = new AST._Waterfall(this, function () {
          this._parent._cases.push({ type: 'compare', test: value, block: this._ast });
          return this._parent;
        });
        this._bind(waterfall);
        return waterfall;
      ...
      this._Match.prototype.WhenType = function () {
        var types = Array.prototype.slice.call(arguments);
        var waterfall = new AST._Waterfall(this, function () {
          this._parent._cases.push({ type: 'kind-of', test: types, block: this._ast });
          return this._parent;
        });
        this._bind(waterfall);
        return waterfall;
      ...
      this._Match.prototype.Otherwise = function () {
        return new AST._Waterfall(this, function () {
          this._parent._otherwise = this._ast;
          return this._parent.end.apply(this._parent, arguments);
        });
      };

      /* Meta Call */
      this._MetaCall = function (parent, end) {
        this._parent   = parent;
        this._changes  = {};
        this.end       = end;
      };

      this._MetaCall.prototype._Waterfall = function (action, selector, glue) {
        if (this._changes[selector] == null) this._changes[selector] = {};
        if (this._changes[selector][action] == null) this._changes[selector][action] = {};
        else throw new Error('Operation already defined');
        var self = this;
        var change = this._changes[selector][action];
        if (arguments.length > 2) change.glue = glue;
        var waterfall = new AST._Waterfall(this, function (level) {
          change.block = this._ast;
          if (level === 0) return this._parent;
      ...
      this._MetaCall.prototype.Append = function (selector, glue) {
        var args = ['append', selector];
        if (arguments.length > 1) args.push(glue);
        return this._Waterfall.apply(this, args);
      };

      this._MetaCall.prototype.Prepend = function (selector, glue) {
        var args = ['prepend', selector];
        if (arguments.length > 1) args.push(glue);
        return this._Waterfall.apply(this, args);
      };

      this._MetaCall.prototype.Replace = function (selector, glue) {
    ...
    this.Runtime.prototype.run = function (ast, payload, callback) {
      ...
      case '[object Object]':
        ...
        if ('_'  in ast) return this.format(ast._, payload, callback);
    ...
    this.Runtime.prototype.execute = function (ast, data, callback) {
      var runtime = this;
      if ('_' in ast)
        return this.format(ast._, data, function (err, data) {
          if (err) return callback(err);
          return runtime.execute({ $: ast.$ }, data, callback);
        });
      switch (typeof ast.$) {
      case 'string': return this.resolve(ast, data, callback);
    ...
    this.Runtime.prototype.format = function (ast, data, callback) {
      return this.run(ast, data, callback);
    };

    this.Runtime.prototype.resolve = function (ast, data, callback) {
      var offset = ast.$.indexOf(':');
      if (offset < 0) return callback(null, ast.$);
      var scheme = ast.$.substr(0, offset);
      var firstChar = scheme.charCodeAt(0);
      if (isNaN(firstChar) || (firstChar > 64 && firstChar < 90) || firstChar == 46) {
        if (ast.$changes == null) return this.node.send(ast.$, data, callback);
        return this.sendSpe(ast, data, callback);
      } else if (scheme in Yolo.VM.DSL) {
        return Yolo.VM.DSL[scheme].call(this, ast, data, callback);
      } else if (scheme in this.node.dsl) {
        return this.node.dsl[scheme].call(this.node, ast, data, callback);
      } else {
        return callback(null, ast.$);
      }
    };

    this.Runtime.prototype.sendSpe = function (ast, data, callback) {
      var changes = { names: ast.$changes, payload: data };
    ...
    this.Runtime.prototype.block = (function () {
      var get = function (runtime, change, data, payload) {
        var slice = [];
        if ('glue' in change) {
      ...
      return function (ast, data, callback) {
        if (!this.__changes) return this.run(ast.block, data, callback);
        var changes = this.__changes.names[ast.name];
        if (changes == null) return this.run(ast.block, data, callback);
        var payload = this.__changes.payload;
        var waterfall = [];
        if (changes.prepend) Array.prototype.push.apply(waterfall, get(this, changes.prepend, data, payload));
        if (changes.replace) Array.prototype.push.apply(waterfall, get(this, changes.replace, data, payload));
        else waterfall.push(ast.block);
        if (changes.append) Array.prototype.push.apply(waterfall, get(this, changes.append, data, payload));
    ...
    this.Runtime.prototype.assert = function (ast, data, callback) {
      return this.run(ast, data, function (err, payload) {
        ...
        return callback(Yolo.Util.wrapError(payload.glue));
      });
    };
    ...
    this.Runtime.prototype._post.merge = function (ast, data, callback) {
    ...
    this.DSL = new function () {
      ...
      this['='] = function (ast, data, callback) {
        return callback(null, ast.$.substr(2));
      };

      this['$'] = function (ast, data, callback) {
        var query = ast.$.substr(2);
      ...
      this['%'] = function (ast, data, callback) {
        var runtime = this;
        var query = ast.$.substr(2);
        var result = query.replace(/([$%&#])\{([0-9a-z.]+)\}/, function (_, type, value) {
          switch (type) {
          case '%':
            switch (value) {
  ...
  this.Node.prototype.begin = function (payload) {
    ...
    return new Yolo.VM.AST._Waterfall(null, function (data, callback) {
      if (hasPayload) {
        ...
        return runtime.run(this._ast, payload, callback);
      } else if (arguments.length > 1) {
        return runtime.run(this._ast, data, callback);
      } else if (arguments.length < 1) {
        return this._ast;
      } else {
