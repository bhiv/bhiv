...
module.exports = new function Yolo() {
  ...
  this.Node.prototype.define = function (name, value) {
    if (arguments.length < 2) return this.model.defines[name] || null;
    if (this.model.defines == null) this.model.defines = {};
    this.model.defines[name] = value;
    return this;
  };
  ...
  this.Loader.prototype.require = function (from, to, callback) {
    var chain = this.chain(from, to);
    return this.inflate(chain, this.node, null, null, callback);
  };

  this.Loader.prototype.chain = function (from, to) {
    var parts = to.split('.');
    var procedure = parts.map(function (name, i) {
      var fqn = parts.slice(0, i + 1).join('.');
      return { name: name, layouts: [{ fqn: fqn }] };
    });
