...
module.exports = new function Yolo() {
  ...
  this.Node.prototype.on = function (name, method) {
    ...
    case 2:
      if (!(method instanceof Yolo.Node.Method))
        method = new Yolo.Node.Method(method);
      this.inlets[name] = method;
      return this;
    default:
  ...
  this.Node.prototype.resolve.fn_async = function (fn, data, flux) {
    return fn(data, flux);
  };
  ...
  this.Node.Method.prototype.call = function (stage, data, callback) {
  ...
  this.Flux = new function () {
    ...
    this.create = function (stage, handler) {
      ...
      flux.emit    = _emit;
      flux.has     = _has;
      flux.handler = _makeHandler(stage, handler);
      return flux;
    };
    ...
    var _makeHandler = function (stage, alpha) {
      if (typeof alpha == 'function') {
        var handler = new Yolo.Handler(stage);
  ...
  this.Stage.prototype.run = function (payload, flux) {
    for (var node = this.node, method = null; node != null && method == null; node = node.older)
      method = node.inlets[this.name];
    if (node == null) return flux(null, payload);
    return method.call(this, payload, flux);
  };
