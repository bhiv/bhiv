...
module.exports = new function Yolo() {
  ...
  this.Node.prototype._require = function (event, slice) {
    ...
    event.used.push(slice);
    var as = event.payload.as;
    var fqn = as || event.payload.fqn;
    ...
    return this.require.require(event.payload.fqn, as, function (err, result) {
  ...
  this.Node.prototype._inlet = function (event, slice) {
    return Yolo.Flux.create(this, slice.name, event.handler).stage.run(event.payload);
  };
  ...
  this.Flux = new function () {
    ...
    this.create = function (node, inlet, handler) {
      ...
      flux.stage   = new Yolo.Flux.Stage(node, inlet, flux);
    ...
    this.Stage = function (node, name, flux) {
      this.node    = node;
      this.name    = name;
      this.layout  = node;
    ...
    this.Stage.prototype.next = function () {
      r: switch (this.track) {
      case 'prepare':
        if (this.layout.older == null) this.request = 'execute';
        switch (this.request) {
      ...
      case 'execute':
        if (this.layout.older == null) this.request = 'format';
        switch (this.request) {
      ...
      case 'format': case 'error':
        ...
        case 'exit': this.layout = null; break r;
        default : break r;
        }
      }
