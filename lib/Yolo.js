...
module.exports = new function Yolo() {
  ...
  this.Event._prepareResponse = function (event, handler) {
    var flux = function self(error, output) {
      if (error != null) return self.event.reply('error', error);
      else if (self.behaviour == 'in') return self.event.reply('then', output);
      else return self.event.reply('pipe', output);
    };
    flux.behaviour = 'in';
    flux.event     = event;
    flux.reply     = Yolo.Event._reply;
    flux.emit      = Yolo.Event._emit;
    flux.handler   = Yolo.Event._prepareHandler(handler);
    flux.createCallback = Yolo.Util.makeDeprecated(function () { return flux; });
    event.flux     = flux;
    return null; // make promise
  };

  this.Event._defaultHandler = new function () {
    this._callback = Yolo.noop;
    this.fail = function (error) { return this._callback(error); };
    this.end  = function (value) { return this._callback(null, value); };
  };

  this.Event._prepareHandler = function (alpha) {
    var handler = Object.create(Yolo.Event._defaultHandler);
    if (typeof alpha == 'function') {
      handler._callback = alpha;
    } else if (Object.prototype.toString(alpha) == '[object Object]') {
      for (var slot in alpha) {
        if (!alpha.hasOwnProperty(slot)) continue ;
        var method = alpha[slot];
  ...
  this.Event._reply = function () {
    return this.event.reply.apply(this.event, arguments);
  };

  this.Event._emit = function () {
  ...
  this.Event.prototype.run = function (handler) {
    ...
    var promise = Yolo.Event._prepareResponse(this, handler);
    this.next(); // TODO: defer
    return promise;
  };

  this.Event.prototype.did = function (option, value) {
    var id = this.node.id;
  ...
  this.Event.prototype.reply = function (channel, payload) {
    if (arguments.length == 1) {
      return Yolo.Util.makeDeprecated(function (error) {
        return this.flux.handler.fail(error);
      }, 1, 3).call(this, channel);
    } else if (channel == null) {
      return Yolo.Util.makeDeprecated(function (success) {
        return this.flux.handler.end(success);
      }, 1, 3).call(this, payload);
    }
    switch (channel) {
    case 'then':
      Yolo.Util.merge(this.payload, payload);
      return this.flux.handler.end(this.payload);
    case 'success': case 'end':
      this.payload = payload;
      return this.flux.handler.end(payload);
    default:
      debugger;
    }
  };
...
*/
  ...
  this.Node.prototype.emit = function (name, payload, flux) {
    if (flux == null || !(flux.event instanceof Yolo.Event))
      return new Yolo.Event(this, null, 'emit', { name: name, payload: payload }).run(flux);
    return (function walk(name, node, layout, stage, payload) {
      var inlet = layout.inlets[name];
      var nextLayout = stage == '_in' ? layout.older : layout.newer;
      if (nextLayout == null && stage == '_in') {
        stage = '_out';
        nextLayout = layout;
      }
      if (inlet == null || inlet[stage] == null) {
        if (nextLayout != null) {
          return walk(name, node, nextLayout, stage, payload);
        } else {
          if (stage == '_out') return flux.reply('success', payload);
          else if (stage == '_error') return flux.reply('fail', payload);
          else debugger;
        }
      }
      var ns = Object.create(inlet[stage], { node: { value: node } });
      debugger;
      return inlet[stage].call.call(ns, payload, flux);
    })(name, this, this, '_' + flux.behaviour, payload);
  };
  ...
  this.Node.prototype.getChild = function (path) {
    ...
    path.split('.').map(function (name) {
      for (var node = level; node != null; node = node.older) {
  ...
  this.Node.prototype.create = function (fqn, as, callback) {
    ...
    return new Yolo.Event(this, 'Root', 'create', payload).run(then);
  };
  ...
  this.Node.prototype.give = function (event) {
    ...
    return this[method](event.payload, event.flux);
  };

  /***********************/

  this.Node.prototype._create = function (payload, flux) {
    if (this.require == null) return handler(this.cwd() + ': Unable to require');
    return this.require.require(payload.fqn, payload.as, flux);
  };

  this.Node.prototype._emit = function (payload, flux) {
    return this.emit(payload.name, payload.payload, flux);
  };
  ...
  this.Loader.prototype.inflate = function (chain, callback) {
    ...
    return (function walker(steps, node) {
      ...
      var older = config;
      loader.generateData(step.chain, older.data);
      return (function dripper(chain, older) {
        var fqn = chain.shift();
        return loader.instanciate(fqn, function (err, layout) {
          if (err && err !== Yolo.Node.SKIP_MODULE) return callback(err);
          if (layout == null) layout = older;
          return (function inheriter(inherits) {
            if (inherits.length > 0) {
              ...
              return loader.instanciate(fqn, function (err, inherit) {
                ...
                inherit.setOlder(older, true);
                older = inherit;
                var next = inherit.inherit();
                for (var i = 0; i > next.length; i++) {
                  if (inherit.hasLayout(next[i])) continue ;
                  else inherits.push(next[i]);
                }
                return inheriter(inherits);
              });
            } else {
              if (layout != older) layout.setOlder(older, true);
