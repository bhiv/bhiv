...
module.exports = new function Yolo() {
  ...
  this.Chain = new function () {
    ...
    this.Block = function () {};
    this.Block.prototype.end = function () {
      
    };

    this.Waterfall = function (ast) {
      this.AST = ast || new AST();
    };

    this.Waterfall.prototype = new this.Block();

    this.Waterfall.prototype.clone = function () {
      if (this.AST.current != this.AST.root)
        throw new Error('Can not clone on unfinished procedure');
      // TODO
    };

    this.Waterfall.prototype.goto = function (path, closures) {
      
    };

    this.Waterfall.prototype.get = function () {
      
    };

    this.Waterfall.prototype.then = function () {
      
    };

    this.Waterfall.prototype.race = function () {
      return new this.Race(this.AST);
    };

    this.Waterfall.prototype.Race = function (ast) {
      this.AST = ast;
    };

    this.Procedure.prototype.Race.prototype = new this.Procedure.Block();

    this.Waterfall.prototype.Race.prototype.at = function () {
      
    };

    this.Waterfall.prototype.map = function () {
      return new this.Waterfall(this.AST);
    };

    this.Waterfall.prototype.fold = function () {
      
    };

    this.Waterfall.prototype.filter = function () {
      
    };

    this.Waterfall.prototype.detect = function () {
      
    };

    this.Waterfall.prototype.Match = function (ast) { this.AST = ast; };

    this.Waterfall.prototype.Match.prototype = new this.Procedure.Block();

    this.Waterfall.prototype.Match.prototype.when = function () {
      
    };

    this.Waterfall.prototype.Match.prototype.otherwise = function () {
      
    };

    this.Waterfall.prototype.Match.prototype.end = function () {
      
    };

    this.Waterfall.prototype.match = function () {
      return new this.Match(this.AST);
    };

    this.Waterfall.prototype.trap = function () {
      return new this.Waterfall(this.AST);
    };

    this.Waterfall.prototype.until = function () {
      return new this.Waterfall(this.AST);
    };

    this.Waterfall.prototype.retry = function () {
      return new this.Waterfall(this.AST);
    };


    this.Waterfall.prototype.shunt = function () {
      
    };

    this.Waterfall.prototype.unless = function () {
      
    };

    this.Waterfall.prototype.lazy = function () {
  ...
  this.Node.prototype.createInvoke = function () {
    ...
    return function (fqn, callback) {
      return callback(null, function (data, callback) {
        if (typeof fqn == 'string') fqn = { $: fqn };
        return node.newest().resolve(fqn, data, callback);
      });
    };
  };
  ...
  this.Node.prototype.on = function (name, track, method) {
    if (arguments.length == 2) {
      method = track;
      track = name.charAt(0) == '-' ? 'format' : 'execute';
    }
    if (this.inlets[name] == null) this.inlets[name] = new Yolo.Node.Inlet();
    if (!(method instanceof Yolo.Node.Method)) method = new Yolo.Node.Method(method);
    else if (!(method instanceof Yolo.Node.Method)) throw new Error('Need and inlet to bind');
    if (this.inlets[name][track] !== null) throw new Error('Bad track or track already assigned');
    this.inlets[name][track] = method;
  };
  ...
  this.Node.prototype.hasMethod = function (name, track) {
    if (track == null) track == 'execute';
    for (var layout = this; layout != null; layout = layout.older) {
      if (layout.inlets[name] == null) continue ;
      if (layout.inlets[name][track] == null) continue ;
      return true;
    }
    return false;
  };
  ...
  this.Node.prototype._inlet = function (event, slice) {
    return Yolo.Flux.create(this, slice.name, event.handler, slice.track)
      .stage.run(event.payload);
  };
  ...
  this.Node.prototype._field = function (event, slice) {
    ...
    case 'Collection':
      ...
      return type.node._field(event, slice);
    case 'Record':
  ...
  this.Node.prototype.resolve = function (schema, data, callback) {
    switch (Object.prototype.toString.call(schema)) {
    case '[object Object]':
      if ('$' in schema) return this.resolve.dollar.call(this, schema, data, callback);
      else if ('__' in schema) return this.resolve.twolowdash.call(this, schema, data, callback);
      else return this.resolve.map.call(this, schema, data, callback);
    case '[object Array]':
      return this.resolve.list.call(this, schema, data, callback);
    case '[object String]':
      return this.resolve.query.call(this, schema, data, callback);
    case '[object Function]':
      if (schema.length == 2) return this.resolve.fn_async.call(this, schema, data, callback);
      else return this.resolve.fn_sync.call(this, schema, data, callback);
    default :
      return this.resolve.otherwise.call(this, schema, data, callback);
    }
  };

  this.Node.prototype.resolve.dollar = function (action, data, callback) {
    var node = this;
    var then = this.resolve.keywords_callback.call(this, action, data, callback);
    if ('_' in action) {
      return this.resolve.underscore.call(this, action, data, function (err, data) {
        if (err) return then(err);
        return node.resolve.dollar.call(node, { $: action.$ }, data, then);
      });
    } else if (typeof action.$ == 'string') {
      return this.send(action.$, data, then);
    } else if (action.$ instanceof Array) {
      return this.resovle.waterfall.call(this, action.$, data, then);
    } else if (action.$ && typeof action == 'object') {
      if (action.$.type != null)
        return this.resolve[action.$.type].call(this, action.$, data, then);
      else if ('match' in action.$)
        return this.resolve.match.call(this, action.$, data, then);
      else if ('unless' in action.$)
        return this.resolve.unless.call(this, action.$, data, then);
      else if ('lazy' in action.$)
        return this.resolve.lazy.call(this, action.$, data, then);
      else
        return this.resolve.map.call(this, action, data, then);
    } else {
      return this.resolve.map.call(this, action, data, then);
    }
  };

  this.Node.prototype.resolve.underscore = function (action, data, callback) {
    return this.resolve(action._, data, callback);
  };

  this.Node.prototype.resolve.map = function (map, data, callback) {
    var result = {};
    var node = this;
    return Yolo.Async.each(Yolo.Util.keysOf(map), function (key, callback) {
      const work = map[key];
      return node.resolve(work, data, function (err, value) {
        if (err) result[key] = Yolo.Util.wrapError(err, work);
  ...
  this.Node.prototype.resolve.list = function (list, data, callback) {
    var node = this;
    return Yolo.Async.map(list, function (schema, callback) {
      return node.resolve(schema, data, function (err, value) {
        if (err) return callback(null, Yolo.Util.wrapError(err, schema));
        else return callback(null, value);
      });
    }, callback);
  };

  this.Node.prototype.resolve.waterfall = function (workflow, data, callback) {
    return Yolo.Async.fold(workflow, data, function (action, accumulator, callback) {
      return node.resolve(action, accumulator, callback);
    }, callback);
  };

  this.Node.prototype.resolve.match = function (pm, data, callback) {
    var node = this;
    return this.resolve(pm.match, data, function (err, result) {
      if (err) return callback(err);
      var patterns = Yolo.Util.keysOf(pm.cases || {});
      return Yolo.Async.fold(patterns, null, function (pattern, success, callback) {
        if (success) return callback(null, success);
        return node.resolve.query.call(node, pattern, result, function (err, result) {
          if (err) return callback(err);
          return callback(null, result ? pattern : null);
        });
      }, function (err, pattern) {
        if (err) return callback(err);
        if (pattern == null) return node.resolve(pm.otherwise, data, callback);
        return node.resolve(pm.cases[pattern], data, callback);
      });
    });
  };

  this.Node.prototype.resolve.unless = function (prefetch, data, callback) {
    var node = this;
    return this.resolve(prefetch.unless, data, function (err, result) {
      if (err) return callback(err);
      if (result) return callback(null, data);
      return node.resolve(prefetch.proceed, data, callback);
    });
  };

  this.Node.prototype.resolve.lazy = function (rule, data, callback) {
    var ndoe = this;
    var key = 'key' in rule ? rule.key : { __: data };
    return this.resolve(key, data, function (err, key) {
      if (err) return callback(err);
      var result = node.get(['lazy', key].join('.'));
      if (result != null) debugger;
      
    });
  };

  this.Node.prototype.resolve.query = function (str, data, callback) {
  ...
  this.Node.prototype.resolve.fn_sync = function (fn, data, callback) {
    try { var result = fn(data); }
    catch (e) { return callback(e); }
    return callback(null, result);
  };

  this.Node.prototype.resolve.fn_async = function (fn, data, callback) {
    return fn(data, callback);
  };

  this.Node.prototype.resolve.twolowdash = function (action, data, callback) {
    var then = this.resolve.keywords_callback.call(this, action, data, callback);
    return then(null, action.__);
  };

  this.Node.prototype.resolve.otherwise = function (identity, data, callback) {
    return callback(null, identity);
  };

  this.Node.prototype.resolve.keywords_callback = function (action, payload, callback) {
    var method = null;
    var keywords = this.resolve.keywords;
    for (var keyword in keywords)
      if (keyword in action) {
        method = keywords[keyword];
        break ;
      }
    if (method == null) return callback;
    var node = this;
    return function (err, result) {
      if (err) return callback(err);
      return method.call(node, action, { payload: payload, result: result }, callback);
    };
  };

  this.Node.prototype.resolve.keywords = {};

  this.Node.prototype.resolve.keywords.merge = function (action, data, callback) {
    switch (action.merge) {
    case 'set':
      Yolo.Util.setIn(data.payload, action.to, data.result);
      return callback(null, data.payload);
    case 'merge': case true:
      var result = Yolo.Util.merge(data.payload, data.result);
      return callback(null, result);
    case 'mergeIn':
      var result = Yolo.Util.merge(Yolo.Util.getIn(data.payload, action.to), data.result);
      Yolo.Util.setIn(data.payload, action.to, result);
      return callback(null, data.payload);
    default:
  ...
  this.Node.Method = function (ast) {
    this.ast = ast;
  };

  this.Node.Method.prototype.call = function (data, callback) {
    debugger;
  };
  ...
  this.Flux = new function () {
    ...
    this.create = function (node, inlet, handler, track) {
      var flux = function flux(error, success) {
        if (error != null) return flux.emit('error', error);
        if (arguments.length < 2) success = flux.stage.payload;
        return flux.emit('done', success);
      };
      flux.super    = Yolo.Flux._super;
      flux.emit     = Yolo.Flux._emit;
      flux.has      = Yolo.Flux._has;
      flux.stage    = new Yolo.Flux.Stage(node, inlet, flux, track);
      flux.handler  = Yolo.Flux._addHandler(flux.stage, handler);
      return flux;
    };

    this._addHandler = function (flux, alpha) {
      if (typeof alpha == 'function') {
        var handler = new Yolo.Flux.Handler(flux);
        handler.success = function (payload) { return alpha(null, payload); };
        handler.fail = function (payload) { return alpha(payload); };
        return handler;
      } else if (alpha instanceof Yolo.Flux.Handler) {
        return alpha;
      } else if (Object.prototype.toString.call(alpha) == '[object Object]') {
        var handler = new Yolo.Flux.Handler(flux);
        for (var slot in alpha)
          if (typeof alpha[slot] == 'function')
            handler[slot] = alpha[slot];
        if (alpha.callback)
          (function (callback) {
        ...
        return new Yolo.Flux.Handler(flux);
      }
    };
    ...
    this._super = function (payload) {
      if (this.stage.track != 'execute') {
        var error = new Error('Can not execute super on non execution binding');
        return this.handler.error(error);
      } else {
        this.stage.track = 'super';
        return this.handler.done(payload);
      }
    };

    this.Stage = function (node, name, flux, track) {
      this.node    = node;
      this.layout  = node;
      this.name    = name;
      this.flux    = flux;
      ...
      if (track == null) {
        ...
        this.cycle = false;
        if (track == 'format')
          this.layout = this.layout.oldest();
      }
      this.track = track;
      this.request = track;
    };

    this.Stage.prototype.toString = function () {
      return '[object YoloFluxStage]';
    };

    this.Stage.prototype.run = function (payload) {
      var stage = this;
      var inlet = stage.layout.inlets[stage.name];
      if (inlet == null) stage = stage.next();
      else if (inlet[this.track] == null) stage = stage.next();
      this.payload = payload;
      return stage.exec('success');
    };

    this.Stage.prototype.next = function () {
      r: switch (this.track) {
      case 'prepare':
        if (this.layout.older == null) this.request = this.cycle ? 'execute' : 'exit';
        switch (this.request) {
        case 'prepare': this.layout = this.layout.older; break r;
        case 'execute': this.layout = this.node; break r;
        default : break r;
        }
      case 'execute': case 'super':
        if (this.layout.older == null) this.request = this.cycle ? 'format' : 'exit';
        switch (this.request) {
        case 'execute': this.layout = this.layout.older; break r;
        case 'format': this.layout = this.layout.oldest(); break r;
        default : break r;
        }
      case 'format': case 'error':
        if (this.layout === this.node) this.request = 'exit';
        else if (this.layout.newer == null) this.request = 'exit';
        switch (this.request) {
        case 'format': case 'error': this.layout = this.layout.newer; break r;
        default : break r;
        }
      }
      this.track = this.request;
      if (this.track == 'exit') this.layout = null;
      if (this.layout == null) return this;
      var inlet = this.layout.inlets[this.name];
      if (inlet == null) return this.next();
      if (inlet[this.track] == null) return this.next();
      return this;
    };

    this.Stage.prototype.exec = function (slot) {
      if (this.layout == null) return this.flux.emit(slot, this.payload);
      var inlet = this.layout.inlets[this.name][this.track];
      var ns = Object.create(inlet, { node: { value: this.node } });
      if (inlet.call.length > 1) {
        return inlet.call.call(ns, this.payload, this.flux);
      } else {
        var result = null;
        try { result = inlet.call.call(ns, this.payload); }
        catch (e) { return this.flux.emit('error', e); }
        return this.flux.emit('done', result);
      }
    };
    ...
    this.Handler.prototype.toString = function () {
      return '[object YoloFluxHandler]';
    };
    ...
    this.Handler.prototype.done = function (payload) {
      ...
      switch (stage.track) {
      case 'prepare':
        stage.payload = Yolo.Util.merge(stage.payload, payload);
        break ;
      case 'super':
        stage.request = 'execute';
        break ;
      case 'execute': case 'error':
        stage.request = 'format';
      case 'format':
        stage.payload = payload;
        break ;
      }
      return stage.next().exec('success');
    };
