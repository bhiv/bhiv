...
module.exports = new function Yolo() {
  ...
  this.Node.prototype.setController = function (controller) {
  ...
  this.Loader.prototype.inflate = function (chain, callback) {
    ...
    return (function walker(steps, node) {
      ...
      var elder = new Yolo.Node('Config', step.name);
      elder.data = loader.generateData(step.chain);
      return (function dripper(chain, elder) {
        var fqn = chain.shift();
        return loader.invoker(fqn, function (err, layout) {
          if (err) return callback(err);
          if (layout == null) layout = elder;
          else layout.setElder(elder);
  ...
  this.Loader.prototype.generateData = function (chain) {
    var data = {};
