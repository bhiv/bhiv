...
module.exports = new function Yolo() {
  ...
  this.Node.prototype.produce = function (name, type) {
    if (arguments.length < 1) return this.fold(function (layout, projections) {
      if (layout.model.projections == null) return projections;
      for (var projection in layout.model.projections)
        if (!~projections.indexOf(projection))
          projections.push(projection);
      return projections;
    }, []);
    if (arguments.length < 2) return this.fold(function (layout, projection) {
      if (projection != null) throw 'break';
      if (layout.model.projections == null) return null;
      return layout.model.projections[name] || null;
    }, null);
    ...
    if (this.model.projections == null) this.model.projections = {};
    this.model.projections[name] = type;
    return this;
  };
  ...
  this.Node.prototype._data = function (event, slice) {
    return event.handler(null, this.get(slice.path));
  };
  ...
  this.Node.prototype._projection = function (event, slice) {
    var schema = this.produce(slice.name);
    if (schema == null) return event.handler(null, null);
    return this.resolve(schema, event.payload, event.handler);
  };
  ...
  this.Node.prototype.resolve.map = function (map, data, callback) {
    ...
    return Yolo.Async.each(Yolo.Util.keysOf(map), function (key, callback) {
      return node.resolve(map[key], data, function (err, value) {
  ...
  this.Node.prototype.resolve.list = function (list, data, callback) {
    ...
    return Yolo.Async.map(list, function (schema, callback) {
      return node.resolve(schema, data, callback);
    }, callback);
  };
  ...
  this.Node.Method = function (fn) {
    this.call = typeof fn == 'function' ? fn : this.sealAST(fn);
  };

  this.Node.Method.prototype.sealAST = function (ast) {
    return function (data, callback) {
      return this.node.resolve(ast, data, callback);
    };
  };
