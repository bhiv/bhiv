...
module.exports = new function Yolo() {
  ...
  this.Cache.prototype.set = function (key, data, expire) {
    ...
    this._map[key] = { expire: date, data: data };
  ...
  this.Node.prototype.cwp = function () {
    var result = [];
    var node   = this;
    var parent = this.parent;
    while (parent != null) {
      if (node.child != null) result.unshift(node.child);
      node = parent;
      parent = node.parent;
    }
    return result;
  ...
  this.Node.prototype.on = function (names, inlet) {
    names = ~names.indexOf(' ') ? names.split(/\s+/) : [names];
    for (var i = 0; i < names.length; i++) {
      var name = names[i];
      if (name == false) continue ;
      if (this.inlets[name] == null) this.inlets[name] = [];
      if (typeof inlet == 'function') inlet = { call: inlet };
      this.inlets[name].push(inlet);
    }
    return this;
  };

  this.Node.prototype.one = function (names, inlet) {
    if (typeof inlet == 'function') inlet = { call: inlet };
    var oneInlet = { count: 1, inlet: inlet, call: function (payload, event) {
      if (!(this.count > 0)) return event.reply();
      this.count -= 1;
      return this.inlet.call(payload, event);
    } };
    return this.on(names, oneInlet);
  };

  this.Node.prototype.off = function (names) {
    names = ~names.indexOf(' ') ? names.split(/\s+/) : [names];
    for (var elder = this.self(); elder != null; elder = elder.elder) {
      for (var i = 0; i < names.length; i++) {
        var name = names[i];
        if (name == false) continue ;
        if (elder.inlets[name] == null) continue ;
        elder.inlets[name] = null;
      }
    }
  };
  ...
  this.Node.prototype.emit = function (name, payload, event) {
    ...
    var inlets = [];
    for (var elder = this.self(); elder != null; elder = elder.elder) {
      var list = elder.inlets[name];
      if (list == null) continue ;
      if (list.length == 0) continue ;
      Array.prototype.unshift.apply(inlets, list);
    }
    var node = this;
  ...
  this.Node.prototype.getChild = function (name) {
    for (var node = this.self(); node != null; node = node.elder)
      if (node.children[name] instanceof Yolo.Node)
  ...
  this.Node.prototype.create = function (path, target, child, callback) {
    var module = this;
    var payload = { target: target };
    var then = function (err, node) {
      if (err) return callback(err);
      if (child) module.attach(node, child);
      return callback(null, node);
    };
    var evt = new Yolo.Event('create', path, { type: 'command', payload: payload, callback: then });
  ...
  this.Node.prototype._require = function (event) {
    ...
    node.create(null, fqn, fqn, function (err, child) {
      if (err) return node._give(event);
  ...
  this.Node.prototype.__create = function (event) {
    if (this.require)
      // event.payload.data
      return this.require(event.payload.target, function (err, node) {
        if (err) return event.reply('fail', err);
        else return event.reply('done', node);
      });
    else
      return event.reply('fail', 'No autoload method');
  };

  /*********************/

  this.Collection = function (indexes) {
    this._indexes = (indexes || []).slice();
    this._maps    = {};
  };

  this.Collection.prototype.has = function (index, value) {
    return this.get(index, value).length > 0;
  };

  this.Collection.prototype.get = function (index, value) {
    if (this._maps[index] == null) return [];
    return this._maps[index][value] || [];
  };

  this.Collection.prototype.getFirst = function (index, value) {
    return this.get(index, value)[0] || null;
  };

  this.Collection.prototype.find = function (filters) {
    var rest = null;
    for (var index in filters) {
      if (!~this._indexes.indexOf(index)) continue ;
      var value = filters[index];
      var set = this.get(index, value);
      if (rest == null) {
        rest = set;
      } else {
        for (var i = 0; i < rest.length;) {
          if (~set.indexOf(rest[i])) {
            i++;
          } else {
            rest.splice(i, 1);
          }
  ...
  this.Collection.prototype.exists = function (content) {
    if (this._indexes.length < 1) return false;
    var set = this.get(this._indexes[0], content[this._indexes[0]]);
    return !!~set.indexOf(content);
  };

  this.Collection.prototype.add = function (content, indexes) {
    var nb = 0;
    if (content == null) return nb;
    if (this.exists(content)) return -1;
    var iter = [content, indexes || {}];
    index: for (var i = 0; i < this._indexes.length; i++) {
      var index = this._indexes[i];
      set: for (var j = 0; j < iter.length; j++) {
        var values = iter[j][index];
        if (values == null) continue set;
        if (!(values instanceof Array)) values = [values];
        value: for (var ii = 0; ii < values.length; ii++) {
          var value = values[ii];
          if (this._maps[index] == null) this._maps[index] = {};
          if (this._maps[index][value] == null) this._maps[index][value] = [];
          this._maps[index][value].push(content);
          nb += 1;
        }
      };
    }
  ...
  this.Collection.prototype.remove = function (content) {
    throw new Error('Bad algo ! something wrong');
    var nb = 0;
    for (var i = 0; i < this._indexes.length; i++) {
      var index = this._indexes[i];
      if (this._maps[index] == null) continue ;
      var offset = this._maps[index].indexOf(content);
      if (offset < 0) continue ;
      this._maps[index].splice(offset, 1);
      nb += 1;
    }
    return nb;
  };
