...
module.exports = new function Yolo() {
  ...
  this.Chain = new function () {
    ...
    this.Block.prototype.end = function () {
      if (this.post) return this.post(this.ast);
      return this;
    };

    this.Waterfall = function (ast, post) {
    ...
    this.Waterfall.prototype.Race = function (ast, post) {
      this.ast  = ast;
      this.post = post;
    };

    this.Waterfall.prototype.Race.prototype = new this.Block();
    ...
    this.Waterfall.prototype.Match = function (ast, post) {
      this.ast  = ast;
      this.post = post;
    };

    this.Waterfall.prototype.Match.prototype = new this.Block();
    ...
    this.Waterfall.prototype.shunt = function (predicate, glue) {
    ...
    this.Waterfall.prototype.memoize = function (duration, glue) {
      var chain = this;
      return new this.Waterfall(null, function (ast) {
  ...
  this.Node.prototype.on = function (name, method) {
    if (this.inlets[name] != null) throw new Error('Method: ' + name + ' already bound');
    switch (arguments.length) {
    case 1:
      var node = this;
      method = new Yolo.Method();
      this.inlets[name] = method;
      return new Yolo.Chain.Waterfall(null, function (ast) {
        method.ast = ast.root;
        return node;
      });
    case 2:
      if (!(method instanceof Yolo.Method))
        method = new Yolo.Method(method);
      this.inlets[name] = method;
      return this;
    default:
  ...
  this.Node.prototype._inlet = function (event, slice) {
    var stage = new Yolo.Stage(this, slice.name);
    var flux  = Yolo.Flux.create(stage, event.handler)
    return stage.run(event.payload, flux);
  };
  ...
  this.Node.prototype.resolve = function (schema, data, callback) {
    ...
    case '[object Function]':
      // because it can not be serialized
      return callback(new Error('YOLO: Function resolution is disabled'));
    default :
  ...
  this.Node.prototype.resolve.dollar = function (action, data, callback) {
    var node = this;
    if ('$$' in action) callback = this.resolve.callback.call(this, action.$$, data, callback);
    if ('$_' in action) this.resolve.options.call(this, action.$_, data);
    if ('_' in action) {
      return this.resolve.underscore.call(this, action, data, function (err, data) {
        if (err) return callback(err);
        return node.resolve.dollar.call(node, { $: action.$ }, data, callback);
      });
    } else if (typeof action.$ == 'string') {
      return this.send(action.$, data, callback);
    } else if (action.$ instanceof Array) {
      return this.resolve.waterfall.call(this, action.$, data, callback);
    } else if (action.$ && typeof action.$ == 'object') {
      if (action.$.type != null && action.$.type in this.resolve.patterns)
        return this.resolve.patterns[action.$.type].call(this, action.$, data, callback);
      else
        return callback(new Error('YOLO: Try to resolve a unknown pattern'));
    } else {
      return this.resolve.map.call(this, action, data, callback);
    }
  };
  ...
  this.Node.prototype.resolve.twolowdash = function (action, data, callback) {
    if ('$$' in action) callback = this.resolve.callback.call(this, action.$$, data, callback);
    return callback(null, action.__);
  };
  ...
  this.Node.prototype.resolve.options = function (options, data) {
    if (options.debug) debugger;
  };

  this.Node.prototype.resolve.patterns = {};

  this.Node.prototype.resolve.patterns.match = function (pm, data, callback) {
    var node = this;
  ...
  this.Node.prototype.resolve.patterns.unless = function (prefetch, data, callback) {
    var node = this;
  ...
  this.Node.prototype.resolve.patterns.error = function (action, data, callback) {
    return this.resolve(action, data, function (err, payload) {
      if (err) return callback(err);
  ...
  this.Node.prototype.resolve.patterns.memoize = function (rule, data, callback) {
    var node = this;
    var key = 'key' in rule ? rule.key : { __: data };
    var memo = node.get('__memoize');
    if (memo == null) node.set('__memoize', memo = new Cache(rule.expire));
    return this.resolve(key, data, function (err, key) {
      if (err) return callback(err);
      var digest = Yolo.Digest(key);
      ...
      if (work != null) {
        ...
        return node.resolve(rule.then, data, function (err, value) {
          var work = memo.pick(digest);
          if (err == null) memo.set(digest, { type: 'resault', value: value });
  ...
  this.Node.prototype.resolve.callback = function (action, payload, callback) {
    var node = this;
    var keywords = this.resolve.keywords;
    for (var keyword in keywords) {
      if (!(keyword in action)) continue ;
      callback = (function (method, payload, callback) {
        return function (error, result) {
          var data = { error: error, payload: payload, result: result };
          return method.call(node, action, data, callback);
        };
      })(keywords[keyword], payload, callback);
    }
    return callback;
  };

  this.Node.prototype.resolve.keywords = {}; // last wrap is first execution

  this.Node.prototype.resolve.keywords.merge = function (action, data, callback) {
    if (data.error) return callback(data.error);
    switch (action.merge) {
    case 'wrap':
      return this.resolve(action.merge_glue, data, callback);
    case 'root':
      var result = Yolo.Util.merge(data.payload, data.result);
      return callback(null, result);
    case 'into':
      var result = Yolo.Util.merge(Yolo.Util.getIn(data.payload, action.merge_to), data.result);
      Yolo.Util.setIn(data.payload, action.merge_to, result);
      return callback(null, data.payload);
    case 'set':
      Yolo.Util.setIn(data.payload, action.merge_to, data.result);
      return callback(null, data.payload);
    case 'skip':
      return callback(null, data.payload);
    default:
  ...
  this.Node.prototype.resolve.keywords.trap = function (action, data, callback) {
    if (data.error == null) return callback(data.error, data.result);
    if (data.error instanceof Error) data.message = data.error.message;
    if (action.trap_if == null) return this.resolve(action.trap, data, callback);
    var node = this;
    return this.resolve(action.trap_if.match, data, function (err, result) {
      if (err) return callback(err);
      var pattern = action.trap_if.pattern;
      if (pattern instanceof RegExp) {
        if (pattern.test(result))
          return node.resolve(action.trap, data, callback);
      } else {
        debugger;
  ...
  this.Method.Context = function (stage) {
    this.stage = stage;
    this.node = stage.root;
  };

  this.Method.Context.prototype.super = function (payload, callback) {
    return this.stage.run(payload, callback);
  };

  this.Method.Context.prototype.next = function (callback) {
    var self = this;
  ...
  this.Method.prototype.call = function (stage, payload, flux) {
    if (typeof this.ast == 'function') {
      var context = new Yolo.Method.Context(stage);
      switch (this.ast.length) {
      case 0: case 1:
        try { var result = this.ast.call(context, payload); }
        catch (e) { return flux(e); }
        return flux(null, result);
      case 2:
        return this.ast.call(context, payload, flux);
      default:
        debugger;
        return flux(new Error('Unknown how to handle this kind of function'));
      }
    } else {
      return stage.root.resolve(this.ast, payload, flux);
    }
  };
  ...
  this.Flux = new function () {
    ...
    this.create = function (stage, handler) {
      ...
      flux.emit    = _emit;
      flux.has     = _has;
      flux.handler = new Yolo.Handler(stage, handler);
      return flux;
    };
  ...
  this.Stage = function (node, name) {
    this.root = node;
    this.next = node;
    this.name = name;
  };

  this.Stage.prototype.run = function (payload, callback) {
    var method = null;
    while (this.next != null) {
      method = this.next.inlets[this.name];
      this.next = this.next.older;
      if (method != null)
        return method.call(this, payload, callback);
    }
