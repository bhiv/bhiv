...
module.exports = new function Yolo() {
  ...
  this.Node.prototype.resolve = function (schema, data, callback) {
    ...
    case '[object Object]':
      var node = this;
      if (schema.$ != null) {
        return this.resolve(schema._, data, function (err, payload) {
          if (err) return callback(err);
          return node.send(schema.$, payload, callback);
        });
      } else {
        var result = {};
        return Yolo.Async.each(Yolo.Util.keysOf(schema), function (key, callback) {
          return node.resolve(schema[key], data, function (err, value) {
            if (err) return callback(err);
            result[key] = value;
            return callback();
          });
    ...
    case '[object String]':
      if (schema.substr(0, 2) == '${' && schema.substr(-1) == '}') {
        var path = schema.substr(2, schema.length - 3);
        return callback(null, Yolo.Util.getIn(data, path));
      } else {
        return callback(null, schema);
      }
    case '[object Function]':
        return callback(null, schema(data));
    default :
      return callback(null, schema);
    }
  };
