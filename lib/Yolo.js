...
module.exports = new function Yolo() {
  ...
  this.Node.prototype.emit = function (event, payload, callback) {
    return this.send([{ type: 'inlet', name: event }], payload, callback);
  };
  ...
  this.Node.prototype.load = function (payload, callback) {
    if (typeof callback != 'function') callback = Yolo.noop;
    if (this.ready) return callback();
    var node = this;
    var fqn = [{ type: 'inlet', name: 'load' }];
    return this.send(fqn, payload, function (err, response) {
      if (err) return callback(err);
      node.ready = true;
