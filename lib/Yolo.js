...
module.exports = new function Yolo() {
  ...
  this.VM = new function () {
    ...
    this.AST = new function () {
      ...
      this._Waterfall.prototype._Waterfall = function (parent, end) {
        var waterfall = new AST._Waterfall(parent, end);
        waterfall._syntaxes = this._syntaxes.slice();
        for (var i = 0; i < this._syntaxes.length; i++) {
          var syntax = this._syntaxes[i];
          for (var m in syntax.module)
            waterfall[syntax.prefix + '_' + m] = syntax.module[m];
        }
        return waterfall;
      };
      ...
      this._Waterfall.prototype.debug = function () {
        return this._push(function (flow) { debugger; return flow; });
      };

      // FIXEME: May deplace it into boostrap.js
      this._Waterfall.prototype.load = function (filepath, prefix) {
        if (prefix == null) prefix = '';
        var errline = (new Error().stack).split(/\r?\n/).slice(2,3);
        var caller = /\((.+):\d+:\d+\)/.exec(errline);
        if (caller == null) throw new Error('Unable to define caller position');
        var parts = caller[1].split('/');
        for (var depth = parts.length - 1; depth > 0; depth--)
          if (parts[depth] == 'app')
            break ;
        var root = parts.slice(0, depth).join('/');
        var modulePath = [root, filepath].join('/');
        var module = require(modulePath).default;
        this._syntaxes.push({ prefix: prefix, module: module });
        return this._Waterfall(this, function () {
          return this._parent.end();
        });
      };

      this._Waterfall.prototype.flow = (function () {
        var methods = { protect: true, copy: true };
        return function (method) {
          if (!methods[method]) throw new Error('Method ' + method + ' to alter flow not defined');
          return this._push({ $: { type: 'alterFlow', method: method } });
        };
      })();
      ...
      this._Waterfall.prototype.as = function (glue) {
        return this._push({ _: glue })._merger();
      };
      ...
      this._Waterfall.prototype.assert = function (test, glue, code) {
      ...
      this._Waterfall.prototype.Require = function (fqn, name, lifetime) {
        assert.isFqn(fqn);
        if (lifetime == null) lifetime = -1;
        return this._Waterfall(this, function () {
          var ast = { $: { type: 'require'
                         , fqn: fqn, name: name, lifetime: lifetime
                         , data: this._ast == NULL ? {} : this._ast
                         }
                    };
          return this._parent._push(ast);
        });
      ...
      this._Waterfall.prototype.use = function (fqn) {
        assert.isFqn(fqn);
        // TODO
      };
      ...
      this._Merger.prototype.dismiss = function () {
        this._current.$$ = { type: 'merge', mode: 'dismiss' };
      ...
      this._Merger.prototype.merge = function (path, glue) {
        if (arguments.length == 0) {
          this._current.$$ = { type: 'merge', mode: 'root' };
        } else if (Yolo.Util.isObject(path)) {
          this._current.$$ = { type: 'merge', mode: 'root', with: path };
        } else {
          this._current.$$ = { type: 'merge', mode: 'into', to: path };
          if (arguments.length > 1) this._current.$$.from = glue;
        }
        return this._unwrapped;
      };
      ...
      this._Match.prototype.WhenEquiv = function (value) {
        var waterfall = new AST._Waterfall(this, function (cont) {
          this._parent._cases.push({ type: 'compare', test: value, block: this._ast });
          return cont ? this._parent : this._parent.end();
        });
        this._bind(waterfall);
        return waterfall;
      };
      ...
      this._Match.prototype.Otherwise = function () {
        return new AST._Waterfall(this, function (cont) {
    ...
    this.DSL = new function () {
      ...
      this['%'] = function (dsl, data, callback) {
        ...
        var result = query.replace(/([=$%#&])\{(.+?)\}/g, function (_, type, expr) {
          ...
          case '&':
            var name = expr.substr(0, expr.indexOf(':')).split('.').reverse();
            var path = expr.substr(expr.indexOf(':') + 1);
