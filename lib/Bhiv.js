...
module.exports = new function Bhiv() {
  ...
  this.VM = new function () {
    ...
    this.Runtime._resolveDynamicCall = function (ast, flow) {
      ...
      ast = Bhiv.Util.inherit(ast);
      ast.$ = ast.$.replace(/(^|\.)\{(.+?)\}|(:)(\{(.+?)\})/g, function (_, pn, n, pm, m) {
        if (n) return pn + Bhiv.Util.name(Bhiv.Util.getIn(flow, n));
        if (m) return pm + Bhiv.Util.method(Bhiv.Util.getIn(flow, m));
        return null;
      });
      return ast;
    };
    ...
    this.Runtime.prototype.execute = function (ast, data, callback) {
      var runtime = this;
      ast = Bhiv.VM.Runtime._resolveDynamicCall(ast, data);
      if ('_' in ast)
