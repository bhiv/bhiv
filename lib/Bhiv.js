/**!
 * Author: npelletier at apiculteur dot io
 * Magic: UroxGvT3uDMQCT1va20i43ZZSxo
 */

module.exports = new function Bhiv() {
  'use strict';
  var Bhiv = this;

  this.debug = false;
  this.noop = function () {};
  this.create = function () { return new Bhiv.Node('SideLess', 'Root'); };

  this.Util = new function () {

    this.onReady = function (fn) {
      if (this.Bhiv != null) return this.Bhiv.Util.defer(fn);
      if (typeof this.BhivOnReady == 'undefined') this.BhivOnReady = [fn];
      else this.BhivOnReady.push(fn);
    };
    ...
    this.setIn = function (target, path, value) {
      ...
      while (chain.length > 0) {
        ...
        } else if (target.hasOwnProperty(chain[0]) && Bhiv.Util.isObject(target[chain[0]])) {
          target = target[chain.shift()];
        } else {
          target = target[chain[0]] = target[chain[0]] ? Bhiv.Util.inherit(target[chain[0]]) : {};
          chain.shift();
        }
      }
      return value;
    ...
    this.serialize = function serialize(data, skipempty, iterator) {
      ...
      case '[object Arguments]':
        if (data._bhiv_serialize != null) return data._bhiv_serialize;
        else data._bhiv_serialize = 'null';
        var result = [];
      ...
      case '[object Object]':
        ...
        if (typeof data.serialize == 'function') {
          ...
          data._bhiv_serialize = 'Circular()';
          var temp = {};
          var order = [];
          for (var i in data) {
            if (i == '_bhiv_serialize') continue ;
            var value = serialize(data[i], skipempty, iterator);
        ...
        if (holder instanceof Array && !replaceArray) {
          ...
          for (var i = 0; i < alpha.length; i++) {
            if (typeof holder[i] == 'object')
              ...
              holder[i] = Bhiv.Util.copy(alpha[i]);
          }
        } else {
          return Bhiv.Util.copy(alpha);
        }
        return holder;
      case '[object Object]':
    ...
    this.isObject = function (alpha) {
      ...
      if (Bhiv.Util.kind(alpha) == 'Object') return true;
      return false;
    };
    ...
    this.name = function (name) {
      return (name + '').split('.').map(function (part) {
        return part.split(/[^a-z0-9]+/i).map(function (part) {
    ...
    this.method = function (method) {
      return (method + '').split(/[^a-z0-9]+/i).map(function (part) {
    ...
    this.compare = (function () {
      ...
      return function (left, right) {
        try {
          Bhiv.Util.walk2(left, right, function (path, left, right) {
            if (left == right) return ;
            var lType = Bhiv.Util.kind(left);
            var rType = Bhiv.Util.kind(right);
      ...
      if (e && typeof e == 'object' && e.error instanceof Error) {
        ...
        if (/error/i.test(e.constructor.name)) {
          ...
          if (typeof e == 'string') {
            ...
            else e = { message: Bhiv.Util.serialize(e) };
          }
    ...
    this.debounce = function (fn, delay) {
      ...
      return function () {
        ...
        var key = Bhiv.Digest(args);
        var date = new Date().getTime();
        var expire = date + delay;
        if (cache[key] == null) {
          cache[key] = { expire: expire, count: 1 };
          setTimeout(function (key, context, args) {
            delete cache[key];
    ...
    this.symbol = function (name) {
      if (name == null) name = Bhiv.Util.id(8);
      if (!/^[a-z$_][a-z0-9]*$/i.test(name)) throw new Error('Bad symbol format');
      var symbol = new Function('', 'return new (function ' + name + '(){})();')();
      symbol.constructor.prototype.toString = function () { return 'symbol:' + name; };
      return symbol;
    };
  ...
  this.Async = new function () {
    ...
    this.each = function (collection, iterator, callback) {
      ...
      for (var i = 0; i < count; i++) {
        Bhiv.Util.defer(iterator, collection[i], function (err, value) {
          count -= 1;
          if (err) {
            if (hasFailed) return ;
            hasFailed = true;
    ...
    this.map = function (collection, iterator, callback) {
      ...
      return (function loop(i) {
        if (i == count) return callback(null, result);
        return iterator(collection[i], function (err, value) {
      ...
      for (var i = 0; i < count; i++) (function (item, i) {
        return Bhiv.Util.defer(iterator, item, function (err, value) {
          count -= 1;
          if (err) {
            if (hasFailed) return ;
            hasFailed = true;
    ...
    this.walk = function (data, watcher, callback) {
      return watcher(data, function (err, data) {
        ...
        switch (Object.prototype.toString.call(data)) {
        case '[object Array]': case '[object Arguments]':
          return Bhiv.Async.map(data, function (value, callback) {
            return Bhiv.Async.walk(value, watcher, callback);
          }, callback);
        case '[object Object]':
          var result = {};
          return Bhiv.Async.each(Bhiv.Util.keysOf(data), function (key, callback) {
            return Bhiv.Async.walk(data[key], watcher, function (err, value) {
              if (err) return callback(err);
              result[key] = value;
              return callback();
            });
  ...
  this.Digest.prototype.update = function (data) {
    var content = typeof data == 'string' ? data : Bhiv.Util.serialize(data, true);
  ...
  this.Cache.prototype.set = function (key, data, expire, onexpire) {
  ...
  this.Cache.prototype.gc = function () {
    ...
    for (var i = 0; i < discard.length; i++) {
      if (discard[i].onexpire) discard[i].onexpire(discard[i].data);
      while ((discard[i].queue || []).length > 0) {
        var err = new Error('Bhiv.Cache: untrottle call garbage collected');
  ...
  this.Assert = new function () {
    var assert = this;

    this.hasOneString = function (stringArray, errorMessage) {
      assert.hasOneType(stringArray, 'string', errorMessage);
    };

    this.hasOneType = function (array, typeName, errorMessage) {
      for (var i = 0; i < array.length; i++)
        if (typeof array[i] === typeName)
          return ;
      throw new Error(errorMessage);
    };

    this.isString = function (str, errorMessage) {
    ...
    this.isObjectOrNull = function (object, message) {
  ...
  this.AST = new function () {
    ...
    var util = new function () {
      ...
      this.inherit = function (instance, Inheritance) {
        var Constructor = function () {};
        Constructor.prototype = instance;
        var instance = new Constructor();
        Object.defineProperty(instance, 'constructor', { value: Inheritance });
        for (var method in Inheritance.prototype)
          instance[method] = Inheritance.prototype[method];
        return instance;
      };
    ...
    this.Chain = function (parent, end) {
      this.$chain    = [];
      this.$parent   = parent;
      ...
      if (parent != null && parent.$syntaxes) {
        for (var i = 0; i < parent.$syntaxes.length; i++) {
          ...
          for (var methodName in syntax.module)
            this[syntax.prefix + methodName] = syntax.module[methodName];
        }
      }
      this.end = end;
    };

    this.Chain.prototype.$push = function (ast) {
      this.$chain.push(ast);
      return this;
    };

    this.Chain.prototype.$pushWithMerge = function (ast) {
      this.$push(ast);
      var self = Object.create(this);
      var merger = new AST.Merger(this, ast);
      for (var method in merger) self[method] = merger[method];
      return self;
    };

    this.Chain.prototype.debug = function () {
      return this.$push(function (flow) { debugger; return flow; });
    };

    this.Chain.prototype.flow = (function () {
    ...
    this.Chain.prototype.relocate = function (location) {};

    this.Chain.prototype.as = function (glue) {
      return this.$pushWithMerge({ _: glue });
    };

    this.Chain.prototype.put = function (glue) {
      return this.$pushWithMerge({ __: glue });
    };

    this.Chain.prototype.then = function (identifier, glue) {
      Bhiv.Assert.isIdentifier(identifier);
      var ast = { $: identifier };
      if (arguments.length > 1) ast._ = glue;
      return this.$pushWithMerge(ast);
    };

    this.Chain.prototype.apply = Bhiv.Util.makeDeprecated(function (path, identifier) {
      Bhiv.Assert.isString(path, 'Path must be a string');
      Bhiv.Assert.isIdentifier(identifier);
      var ast = { $: identifier, _: '$:' + path };
      return this.$pushWithMerge(ast).merge(path);
    });

    this.Chain.prototype.defer = function (identifier, glue) {
      Bhiv.Assert.isIdentifier(identifier);
    ...
    this.Chain.prototype.Pipe = function () {
      return new AST.Waterfall(this, function () {
        return this.$parent.$pushWithMerge(this.$ast);
      });
    };

    this.Chain.prototype.Race = function (holder) {
      return new AST.Race(this, function () {
        return this.$parent.$pushWithMerge(this.$ast);
      }, holder);
    };

    this.Chain.prototype.Until = function (test, delay, glue) {
    ...
    this.Chain.prototype.Sideway = function () {
      return new AST.Waterfall(this, function () {
    ...
    this.Chain.prototype.Map = function (source) {
      Bhiv.Assert.isCollection(source);
      return new AST.Waterfall(this, function () {
    ...
    this.Chain.prototype.Reduce = function (source) {
      Bhiv.Assert.isCollection(source);
      return new AST.Waterfall(this, function () {
    ...
    this.Chain.prototype.Filter = function (source) {
      Bhiv.Assert.isCollection(source);
      return new AST.Waterfall(this, function () {
    ...
    this.Chain.prototype.Detect = function (source) {
      return new AST.Waterfall(this, function () {
    ...
    this.Chain.prototype.Fold = function (accu, source) {
      Bhiv.Assert.isCollection(source);
      return new AST.Waterfall(this, function () {
    ...
    /** Flow routing */
    this.Chain.prototype.Match = function (view) {
    ...
    this.Chain.prototype.Unless = function (test) {
      return new AST.Waterfall(this, function () {
    ...
    this.Chain.prototype.memoize = function (duration, key) {
      if (arguments.length < 2) key = '$:@';
      if (arguments.length < 1) duration = 200;
      return new AST.Waterfall(this, function () {
        var ast = { $: { type: 'memoize', expire: duration, key: key, block: this.$ast } };
        var context = this.$parent.$push(ast);
        return context.end.apply(context, arguments);
      });
    };

    this.Chain.prototype.shunt = function (test, then) {
      return new AST.Waterfall(this, function () {
    ...
    this.Chain.prototype.trap = function (test, then) {
      return new AST.Waterfall(this, function () {
    ...
    this.Chain.prototype.assert = function (test, glue, code) {
      var ast = { $: { type: 'assert', test: test } };
      if (glue != null) ast.$.glue = glue;
      if (code != null) ast.$.code = code;
      return this.$push(ast);
    };

    this.Chain.prototype.failWith = function (message) {
      var ast = { $: { type: 'failWith', error: message } };
      return this.$push(ast);
    };

    this.Chain.prototype.Block = function (name, shard) {
      Bhiv.Assert.isString(name, 'Block must be named');
      return new AST.Waterfall(this, function () {
        var ast = { $: { type: 'block', name: name, block: this.$ast } };
        if (shard != null) ast.$.shard = shard;
        return this.$parent.$push(ast);
      });
    };

    this.Chain.prototype.Then = function (fqn, glue) {
      Bhiv.Assert.isFqn(fqn);
      var ast = { $: fqn };
      if (arguments.length > 1) ast._ = glue;
      var modifier = function (chain) { return { $: chain }; };
      return new AST.Template(this, modifier, function () {
    ...
    this.Chain.prototype.Require = function (fqn, name, lifetime) {
      Bhiv.Assert.isFqn(fqn);
    ...
    this.Waterfall = function (parent, end) {
      return util.inherit(new AST.Chain(parent, function () {
        switch (this.$chain.length) {
        case 0:  this.$ast = NULL; break ;
        case 1:  this.$ast = this.$chain[0]; break ;
        default: this.$ast = { $: this.$chain }; break ;
        }
    ...
    /* Merger */
    this.Merger = function (parent, ast) {
    ...
    this.Merger.prototype.merge = function (path, glue) {
      ...
      } else if (Bhiv.Util.isObject(path)) {
    ...
    this.Race.prototype.At = function (path) {
      ...
      var waterfall = new AST.Waterfall(this, function (forMember) {
        Bhiv.Util.setIn(this.$parent.$ast, this.$path, this.$ast);
        var context = this.$parent;
        if (forMember) return context;
        else return context.end();
      });
      waterfall.$path = path;
    ...
    /* Template */
    this.Template = function (parent, modifier, end) {
      this.$parent   = parent;
      this.$changes  = {};
      this.$syntaxes = util.getParentSyntaxes(parent);
    ...
    this.Template.prototype.$Sequence = function (action, selector, glue) {
      ...
      var chain = new AST.Chain(this, function (level) {
        change.block = self.$modifier(this.$chain);
        if (level === 0) return this.$parent;
        return this.$parent.end();
      });
      chain.Replace = function () { return this.end(0).Replace.apply(self, arguments); };
      chain.Prepend = function () { return this.end(0).Prepend.apply(self, arguments); };
      chain.Append  = function () { return this.end(0).Append.apply(self, arguments); };
      return chain;
    };

    this.Template.prototype.Append = function (selector, glue) {
      var args = ['append', selector];
      if (arguments.length > 1) args.push(glue);
      return this.$Sequence.apply(this, args);
    };

    this.Template.prototype.Prepend = function (selector, glue) {
      var args = ['prepend', selector];
      if (arguments.length > 1) args.push(glue);
      return this.$Sequence.apply(this, args);
    };

    this.Template.prototype.Replace = function (selector, glue) {
  ...
  this.VM = new function () {
    ...
    const Runtime = this.Runtime = function (node) {
      this.node = node.newest();
    };

    this.Runtime._getSource = function (flow, source) {
      if (source == null || source == '.') return flow;
      else if (typeof source == 'string') return Bhiv.Util.getIn(flow, source);
      else if (source instanceof Array) return source;
      else return new Error('Bad source');
    };

    this.Runtime._resolveDynamicFqn = function (fqn, flow) {
      const rxp = /^\{(.+?)\}$|(^|\.)\{(.+?)\}([.:])|([:\-])\{(.+?)\}(-|$)/g
      return fqn.replace(rxp, function (_, f, pn, n, an, pm, m, am) {
        if (f) {
          const fqn = Bhiv.Util.getIn(flow, f);
          const fqnp = (fqn || ':-null').split(':');
          return Bhiv.Util.name(fqnp[0]) + ':' + Bhiv.Util.method(fqnp[1]);
        } else if (n) {
          const name = Bhiv.Util.getIn(flow, n);
          return pn + Bhiv.Util.name(name) + an;
        } else if (m) {
          const method = Bhiv.Util.getIn(flow, m);
          return pm + Bhiv.Util.method(method) + am;
        } else {
          return null;
        }
      });
    ...
    this.Runtime.prototype.alterFlow = function (ast, payload, callback) {
      switch (ast.method) {
      case 'copy': return callback(null, Bhiv.Util.copy(payload));
      case 'protect': return callback(null, Bhiv.Util.inherit(payload));
    ...
    this.Runtime.prototype.execute = function (ast, data, callback) {
      var runtime = this;
      var type = typeof ast.$;
      if (type == 'string' && ~ast.$.indexOf('{')) {
        // Have to be resolve before parameter
        ast = Object.create(ast);
        ast.$ = Runtime._resolveDynamicFqn(ast.$, data);
      }
      if ('_' in ast)
    ...
    this.Runtime.prototype.resolveDSL = function (dsl, data, callback) {
      ...
      if (scheme in Bhiv.VM.DSL) {
        return Bhiv.VM.DSL[scheme].call(this, dsl, data, callback);
    ...
    this.Runtime.prototype.block = (function () {
      var set = function (stack, operator, runtime, change, data, payload) {
        ...
        if ('glue' in change) {
          ...
          slice.push(function (data, callback) {
            return runtime.run(glue, { super: payload, this: data }, (err, value) => {
              payload = Bhiv.Util.merge(Bhiv.Util.copy(payload), value);
      ...
      return function (ast, data, callback) {
        ...
        var chain = before.concat(current, after);
        if (ast.shard != null) {
          var shard = Bhiv.Util.copy(ast.shard.ast);
          Bhiv.Util.setIn(shard, ast.shard.at, chain);
          return this.run(shard, data, callback);
        } else if (chain.length == 1) {
          return this.run(chain[0], data, callback);
        } else {
          return this.run({ $: chain }, data, callback);
        }
      };
    })();
    ...
    this.Runtime.prototype.waterfall = function (workflow, data, callback) {
      var runtime = this;
      return Bhiv.Async.fold(workflow, data, function (ast, accumulator, callback) {
    ...
    this.Runtime.prototype.parallelArray = function (list, data, callback) {
      var runtime = this;
      return Bhiv.Async.map(list, function (schema, callback) {
        return runtime.run(schema, data, function (err, value) {
          if (err) {
            err = Bhiv.Util.wrapError(err, schema);
    ...
    this.Runtime.prototype.parallelObject = function (object, data, callback) {
      ...
      if (first == null) return callback(null, result);
      var runtime = this;
      return Bhiv.Async.each(Bhiv.Util.keysOf(object), function (key, callback) {
        var work = object[key];
        return runtime.run(work, data, function (err, value) {
          if (err) {
            err = Bhiv.Util.wrapError(err, work);
            runtime._log('error', err);
            result[key] = err;
          }
    ...
    this.Runtime.prototype.sideway = function (ast, data, callback) {
      Bhiv.Util.defer(function (runtime) { return runtime.run(ast.block, data); }, this);
      return callback(null, data);
    };
    ...
    this.Runtime.prototype.map = function map(ast, data, callback) {
      var runtime = this;
      var source = Bhiv.VM.Runtime._getSource(data, ast.source);
      if (source instanceof Error) return callback(source);
      return Bhiv.Async.map(source, function (value, callback, index) {
    ...
    this.Runtime.prototype.reduce = function reduce(ast, data, callback) {
      var runtime = this;
      var source = Bhiv.VM.Runtime._getSource(data, ast.source).slice();
      if (source instanceof Error) return callback(source);
      ...
      return Bhiv.Async.map(batch, function (subflow, callback) {
    ...
    this.Runtime.prototype.fold = function fold(ast, data, callback) {
      ...
      var source = Bhiv.VM.Runtime._getSource(data, ast.source);
      if (source instanceof Error) return callback(source);
      var accu = Bhiv.Util.copy(ast.accu);
      return Bhiv.Async.fold(source, accu, function (value, accu, callback, index) {
    ...
    this.Runtime.prototype.filter = function filter(ast, data, callback) {
      ...
      if (source instanceof Error) return callback(source);
      var result = [];
      return Bhiv.Async.each(source, function (value, callback, index) {
        var flow = { flow: data, key: index, value: value };
    ...
    this.Runtime.prototype.detect = function detect(ast, data, callback) {
      var runtime = this;
      var source = Bhiv.VM.Runtime._getSource(ast.source, data);
      if (source instanceof Error) return callback(source);
      var succeeded = false;
      var lastError = null;
      return Bhiv.Async.each(source, function (value, cb, index) {
    ...
    this.Runtime.prototype.match = (function () {
      ...
      var match = function (runtime, ast, flow, callback) {
        return Bhiv.Async.fold(ast.cases, null, function (pattern, _, callback) {
          switch (pattern.type) {
          case 'ast':
          ...
          case 'compare':
            if (Bhiv.Util.compare(pattern.test, ast.view) != 0) return callback();
          ...
          case 'kind-of':
            if (!Bhiv.Util.inArray(pattern.test, Bhiv.Util.kind(ast.view))) return callback();
            else return callback(FOUND, pattern.block);
          default:
    ...
    this.Runtime.prototype.memoize = function (rule, data, callback) {
      ...
      if (memo == null) this.node.set('__memoize', memo = new Bhiv.Cache());
      return this.run({ key: key, expire: rule.expire }, data, function (err, info) {
        ...
        var digest = Bhiv.Digest(info.key);
        ...
        if (work != null) {
          ...
          return runtime.run(rule.block, data, function (err, value) {
    ...
    this.Runtime.prototype.trap = function (ast, data, callback) {
      ...
      return runtime.run(ast.block, data, function (error, result) {
        ...
        var subflow = { error: error, payload: data };
    ...
    this.Runtime.prototype.assert = function (ast, data, callback) {
      ...
      return runtime.run(ast.test, data, function (err, succeed) {
        ...
        return runtime.run(ast.glue, data, function (err, error) {
          if (err) return callback(err);
          var result = Bhiv.Util.wrapError(error);
    ...
    this.Runtime.prototype.failWith = function (ast, data, callback) {
      if (ast.error) return callback(Bhiv.Util.wrapError(ast.error));
        ...
        return runtime.run(ast.name, data, function (err, name) {
          ...
          return runtime.run(ast.data, data, function (err, init) {
            ...
            return runtime.node.create(ast.fqn, target, function (err, slice) {
              ...
              dataNode.data = Bhiv.Util.merge(dataNode.data, init);
      ...
      case 'root':
        if ('with' in ast) {
          return this.run(ast.with, data.result, function (err, value) {
            if (err) return callback(err);
            var result = Bhiv.Util.merge(data.payload, value);
            return callback(null, result);
          });
        } else {
          var result = Bhiv.Util.merge(data.payload, data.result);
          return callback(null, result);
        }
      case 'into':
        if ('from' in ast) {
          ...
          var result = Bhiv.Util.merge(Bhiv.Util.getIn(data.payload, ast.to), data.result);
          Bhiv.Util.setIn(data.payload, ast.to, result);
          return callback(null, data.payload);
        }
      case 'set':
        Bhiv.Util.setIn(data.payload, ast.to, data.result);
        return callback(null, data.payload);
      case 'dismiss':
        return callback(null, data.payload);
      case 'push': case 'unshift':
        var array = Bhiv.Util.getIn(data.payload, ast.to);
        if (!(array instanceof Array)) array = [];
        array[ast.mode](data.result);
        Bhiv.Util.setIn(data.payload, ast.to, array);
        return callback(null, data.payload);
      default:
    ...
    this.DSL = new function () {
      ...
      this['$'] = function (dsl, data, callback) {
        ...
        return callback(null, Bhiv.Util.getIn(data, query));
      };
          ...
          case '&':
            ...
            for (var node = runtime.node; node != null; node = node.parent) {
              ...
              for (var i = 0; i < path.length; i++) {
                ...
                var value = Bhiv.Util.getIn(configs[slot], path);
                if (value == null) continue ;
                return value;
              }
            }
            return '';
          case '$': return Bhiv.Util.getIn(data, expr);
          case '=': return expr;
          case '>':
            var separator = '\0yljs\0';
            var tag = separator + Bhiv.Util.id() + separator;
    ...
    this.Flux = new function () {
      ...
      this.create = function (handler) {
        ...
        flux.handler = new Bhiv.VM.Handler(handler);
        return flux;
      };
  ...
  this.Node.create = function (layout, scope) {
    if (layout instanceof Bhiv.Structure) {
      var structure = layout;
      layout = structure._fqn;
      if (layout == null) layout = Bhiv.Node.fqn(structure);
      if (scope == null) {
        scope = {};
        scope[layout] = structure;
      }
    }
    var node = new Bhiv.Node(layout);
    if (scope != null) {
      node.scope = scope;
  ...
  this.Node.fqn = function (structure, prefix) {
    ...
    if (structure == null) return prefix + '.' + Bhiv.Util.id(16);
    var longhash = Bhiv.Digest(structure);
  ...
  this.Node.prototype.toString = function () {
    return '[object BhivNode]';
  };

  this.Node.prototype.build = function (fqn, callback) {
    return this.create(fqn, fqn, callback);
  };
  ...
  this.Node.prototype.cwd = function (payload) {
    ...
    while (self.parent != null && self.require == null) {
      var value = self.name;
      var hub = Bhiv.Util.getIn(payload, self.hub());
  ...
  this.Node.prototype.get = function (path) {
    ...
    } else if (Bhiv.Util.kind(path) == 'Array') {
      var result = {};
      for (var i = 0; i < path.length; i++)
        Bhiv.Util.setIn(result, path[i], this.get(path[i]));
      return result;
    } else {
      return null;
    }
  ...
  this.Node.prototype.set = function (path, value) {
    ...
    while (chain.length > 0) {
      ...
      } else if (Bhiv.Util.isObject(data[chain[0]])) {
  ...
  this.Node.prototype.field = function (name, fqn, options) {
    ...
    // Set name
    if (!Bhiv.Util.isObject(options)) options = { name: name, fqn: fqn, required: !!options };
    if (options.name == null) options.name = name;
    if (options.autofetch == null) options.autofetch = !!options.required;
    if (options.unique == null) options.unique = false;
    else if (options.unique && this.model.identities[name] == null)
  ...
  this.Node.prototype.begin = function (payload) {
    ...
    var runtime = new Bhiv.VM.Runtime(this);
    return new Bhiv.AST.Waterfall(null, function (data, callback) {
      if (hasPayload) {
  ...
  this.Node.prototype.on = function (name, ast) {
    ...
    case 1:
      var node = this;
      var error = new Error('Method :' + name + ' not ended');
      Bhiv.Util.defer(function () { if (error) throw error; });
      return new Bhiv.AST.Waterfall(null, function () {
        error = null;
  ...
  this.Node.prototype.execute = function (inletName, payload, callback) {
  ...
  this.Node.prototype.spread = function (event, payload, callback) {
    return this.mapreduce(function (node, _, callback) {
      node.execute(event, payload, callback);
    }, null, callback);
  };
  ...
  this.Node.prototype.setOlder = function (older, atEnd) {
    if (!(older instanceof Bhiv.Node)) throw new Error('Bad node type');
    var node = this;
    if (atEnd === true)
    ...
    node.older = older;
    older.newer = node;
    return this.execute('-set-older', older);
  };

  this.Node.prototype.setParent = function (parent) {
    if (!(parent instanceof Bhiv.Node)) throw new Error('Bad node type');
    this.parent = parent;
    return this.execute('-set-parent', parent);
  };
  ...
  this.Node.prototype.mapreduce = function (map, reduce, callback) {
    ...
    (function loop(names, node) {
      ...
      if (names.length > 0) return Bhiv.Util.defer(loop, names, node);
    })(names, this);
    return this;
  };
  ...
  this.Node.prototype.send = function (fqn/*, changes*/, payload, callback) {
    ...
    if (typeof fqn == 'string') fqn = Bhiv.Fqn.parse(fqn);
    if (arguments.length == 4) {
      fqn[fqn.length - 1].changes = payload;
      payload = callback;
      callback = arguments[3];
    }
    return new Bhiv.Event(this, fqn, payload).run(callback);
  };
    ...
    return function (event, slice) {
      ...
      return this.require.require(from(this, event.payload.fqn), as, function (err, result) {
        ...
        if (typeof as == 'string') {
          ...
          return result.leaf.execute('-load', result, reply);
        }
  ...
  this.Node.prototype._create = function (event, slice) {
    ...
    var fqn = Bhiv.Loader.extract(forward).fqn;
    ...
    return this.create(fqn, fqn, function (err, node) {
      ...
      if (err) {
        if (event.handler) return event.handler(err);
  ...
  this.Node.prototype._inlet = function (event, slice) {
    var runtime = new Bhiv.Node.InletRuntime(this, slice.name, slice.changes);
    var flux  = Bhiv.VM.Flux.create(event.handler);
    return runtime.super(event.payload, flux);
  };

  this.Node.InletRuntime = function (node, name, changes) {
    Bhiv.VM.Runtime.call(this, node);
    this.__next    = node;
    this.__name    = name;
    this.__changes = changes;
  };

  this.Node.InletRuntime.prototype = Bhiv.Util.inherit(Bhiv.VM.Runtime.prototype);
    ...
    var produce = function produce(tree, payload) {
      ...
      for (var i = 0; i < tree.length; i++) {
        ...
        case 'dynamic':
          ...
          var subvalue = Bhiv.Util.getIn(payload, subpath);
          path.push(subvalue);
          break ;
        }
      }
 ...
 /**************************/
  ...
  this.Node.prototype.create = function (fqn, as, callback) {
    ...
    if (typeof callback != 'function') callback = Bhiv.noop;
  ...
  this.Event = function Event(node, fqn, payload) {
    ...
    if (!(node instanceof Bhiv.Node)) throw new Error('Need a node to bring an event');
  ...
  this.Loader.extract = function (fqn) {
    if (typeof fqn == 'string') fqn = Bhiv.Fqn.parse(fqn);
    var last = null;
    var base = Bhiv.Fqn.stringify(fqn, function (slice, i) {
      switch (slice.type) {
    ...
    var args = (last.args || []).map(function (arg) {
      return Bhiv.Fqn.stringify(arg);
    });
    return { fqn: base, args: args };
  };

  this.Loader.chain = function (from, to) {
    var fqn = Bhiv.Fqn.parse(to);
    var procedure = [];
    Bhiv.Fqn.stringify(fqn, function (slice, i) {
      if (slice.type != 'child') return ;
      var layout = Bhiv.Loader.extract(fqn.slice(0, i + 1))
    ...
    layouts = layouts.map(Bhiv.Loader.extract);
  ...
  this.Loader.prototype.require = function (from, to, callback) {
    if (to == null) to = from;
    var chain = Bhiv.Loader.chain(from, to);
  ...
  this.Loader.prototype.inflate = function (chain, trunk, branch, leaf, callback) {
    if (chain.length == 0) {
      ...
      return result.leaf.execute('-init', result, callback);
    }
    ...
    var config = new Bhiv.Node('$', step.name);
  ...
  this.Loader.prototype.generateData = function (layout) {
    var data = {};
    var paths = Bhiv.Loader.generateDataPaths(layout);
    for (var i = 0; i < paths.length; i++) {
      var config = this.configs[paths[i]];
      if (config == null) continue ;
      data = Bhiv.Util.merge(data, config);
    }
    return data;
  };
  ...
  Bhiv.Util.defer(function () {
    if (this.BhivOnReady == null) return ;
    while (this.BhivOnReady.length > 0) Bhiv.Util.defer(this.BhivOnReady.shift());
    this.BhivOnReady == null;
  });
