...
(function YoloBootstrap() {
  ...
  var pattern = path.join(__dirname, '../config/config-*.json');
  ...
  var configs =
    ...
    , path.join(__dirname, '../config/config-custom.json')
    , path.join(__dirname, '../config/config-' + environ + '.json')
    , path.join(__dirname, '../config/config-' + hostname + '.json')
    , path.join(__dirname, '../config/config-' + launcher + '.json')
    ];
  ...
  Modules.require = function (fqn, callback) {
    ...
    return Module.invoke(fqn, node, function (err, node) {
      if (err) {
        logger.notice(err);
        return callback(err);
      } else {
