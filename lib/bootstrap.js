...
(function YoloBootstrap() {
      ...
      parents.map(function (elderpath) {
        ...
        try {
          if (fs.lstatSync(elderpath).isDirectory()) {
            ...
            logger.warn('disarding dependency %s', elderpath);
          }
        } catch (e) {
          logger.warn('disarding dependency %s', elderpath);
        }
  ...
  var tasks = [];
  var configs = []
  var chain = ['default', environ, hostname, null, null, launcher];
  ...
  for (var i = 0; i < configChain.length; i++) {
    ...
    for (var fqn in config.Instances) {
      ...
      Yolo.Util.merge(instances[fqn].engines, instance.engines);
    }
  }
  ...
  Root.on('debug', function (message) { logger.debug(message); });
  Root.on('info', function (message) { logger.info(message); });
  Root.on('log', function (message) { logger.log(message); });
  Root.on('notice', function (message) { logger.notice(message); });
  Root.on('trace', function (message) { logger.trace(message); });
  Root.on('alert', function (message) { logger.alert(message); });
  Root.on('warn', function (message) { logger.warn(message); });
  Root.on('error', function (message) { logger.error(message); });
  Root.on('critical', function (message) { logger.critical(message); });
  Root.on('emergency', function (message) { logger.emergency(message); });
  ...
  setTimeout(function () {
    return async.mapLimit(Object.keys(instances), 1, function (name, cb) {
      ...
      return async.map(nodes, function (node, cb) {
        ...
        return node.emit('init', node, function (err) {
          if (err) {
