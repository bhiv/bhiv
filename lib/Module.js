    ...
    Yolo.Util.defer(function () {
      return fs.open(filepath, 'r', function (e, fd) {
        ...
        return fs.read(fd, buf, 0, 1024, 0, function (e) {
          ...
          logger.warn(filepath, 'does not have magic number');
          logger.warn('Please insert /*!UroxGvT3uDMQCT1va20i43ZZSxo*/ at the begining');
        });
      });
    });
  };
  ...
  function createGrower(fqn, args) {
    var grower = function grower() {
      ...
      for (var i = 0; i < grower.calls.length; i++) {
        ...
        grower.calls[i].default.call(grower, layout, logger, Bee, jp);
