...
module.exports = function (dependencies) {
  ...
  return function invoke(fqn, callback) {
    ...
    return getExistingFiles(locations, function (err, locations) {
      ...
      if (locations.length == 0) return callback(Yolo.Node.SKIP_MODULE);
  ...
  function proceedRequires(fqn, locations, callback) {
    ...
    for (var i = 0; i < locations.length; i++) {
      ...
      if (module == null) {
        logger.notice('Module %s found but no grower declared', fqn);
        return callback(Yolo.Node.SKIP_MODULE);
      } else if (typeof module.default != 'function') {
        logger.notice('Module %s found but grower is not a function', fqn);
        return callback(Yolo.Node.SKIP_MODULE);
      }
